...
test.describe('WebSocket close/terminate failure injection', () => {
  test('close() captures ws.close() exception and throws AggregateError; pending waiter is settled', async () => {
    ...
    try {
      ...
      expect(captured, 'close() must reject when ws.close() throws').toBeDefined();
      ...
      expect(messages.some((m) => /injected ws.close failure/.test(m))).toBe(true);
      try {
        await waiter;
  ...
  test('close() captures ws.terminate() exception when graceful close fails', async () => {
    ...
    try {
      const s = await openApiOnlySocket(srv.url, 2_000);
      Object.defineProperty(s.ws, 'readyState', { configurable: true, get: () => 1 });
      s.ws.terminate = () => {
      ...
      const agg = err as AggregateError;
      const messages = agg.errors.map((e) => e.message);
      expect(messages.some((m) => /injected ws.terminate failure/.test(m))).toBe(true);
      try {
        await waiter;
      } catch {
        // expected
