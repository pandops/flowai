...
test('isAlreadyRemovedError does NOT treat "already in use" as removed', () => {
  const err = new ContainerRuntimeError('docker rm --force abc', 1, '', 'Error: another process is already in use');
...
test('ContainerRuntimeError preserves the original command and exitCode in sanitized form', () => {
  const err = new ContainerRuntimeError('docker rm --force abc', 125, '', 'container not found');
