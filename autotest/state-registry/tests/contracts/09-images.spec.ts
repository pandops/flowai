...
test('v0002.50 claim response resolves teams.default_image when task_types and source_systems have no default', async () => {
  ...
  try {
    ...
    const teamBody = await team.json() as { team_id: string; default_image: ImageReference };
    teamId = teamBody.team_id;
    expect(teamBody.default_image, 'admin create returns the persisted ImageReference object').toEqual(defaultImage);
  ...
  await test.step('claim response records resolved_image from teams.default_image with image_source = team_default', async () => {
    ...
    try {
      const claim = await api.post(`/v1/executors/${encodeURIComponent(exec.attach()['X-FlowAI-Executor-Id'] ?? '')}/claim`, {
      ...
      expect(body.resolved_image, 'claim resolved_image is the team default object').toEqual(defaultImage);
      expect(body.image_source, 'image_source is team_default when only the team default applies').toBe('team_default');
      expect(body.task.resolved_image, 'task envelope resolved_image is the team default object').toEqual(defaultImage);
      expect(body.task.image_source, 'task envelope image_source is team_default').toBe('team_default');
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.51 task-level image override wins over team default and is preserved across retries', async () => {
  ...
  try {
    const team = await adminApi.post('/admin/teams', {
      data: { team_name: `${suffix}-a`, default_image: imageReference(`default-A-${suffix}`) },
    });
    expect(team.status()).toBe(201);
  ...
  try {
    ...
    expect(resp.status(), 'first POST /v1/tasks creates the canonical pending task with 201').toBe(201);
    const body = await resp.json() as { task_id: string; image: ImageReference };
  ...
  await test.step('listener retry preserves the original override and does NOT replace it (returns 200)', async () => {
    ...
    try {
      ...
      expect(retry.status(), 'idempotent retry returns 200 with the existing canonical task').toBe(200);
      const body = await retry.json() as { task_id: string; image: ImageReference };
      expect(body.task_id, 'retry returns the same canonical task').toBe(firstTaskId);
      expect(body.image, 'image override is immutable across retries').toEqual(firstImage);
    } finally {
...
test('v0002.74 task_types.default_image wins when tasks.image is null', async () => {
  ...
  let taskTypeId = '';
  let listenerIdentity = '';
  let taskTypeDefaultImage: ImageReference = imageReference(`default-TT-${suffix}`);
  try {
    const team = await adminApi.post('/admin/teams', {
      data: { team_name: `${suffix}-a`, default_image: imageReference(`default-A-${suffix}`) },
    });
  ...
  await test.step('claim response carries resolved_image from task_types.default_image with image_source = task_type_default', async () => {
    ...
    try {
      ...
      expect(claim.status(), 'claim returns 200 with the canonical claim response').toBe(200);
      const body = await claim.json() as { resolved_image: ImageReference; image_source: string };
      expect(body.image_source, 'task_types.default_image wins over team default').toBe('task_type_default');
      expect(body.resolved_image, 'resolved_image is the task-type default object').toEqual(taskTypeDefaultImage);
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.75 source_systems.default_image wins when tasks.image and task_types.default_image are null', async () => {
  ...
  let taskTypeId = '';
  let listenerIdentity = '';
  let sourceSystemDefaultImage: ImageReference = imageReference(`default-SS-${suffix}`);
  try {
    const team = await adminApi.post('/admin/teams', {
      data: { team_name: `${suffix}-a`, default_image: imageReference(`default-A-${suffix}`) },
    });
  ...
  await test.step('claim response carries resolved_image from source_systems.default_image with image_source = source_system_default', async () => {
    ...
    try {
      ...
      expect(claim.status(), 'claim returns 200 with the canonical claim response').toBe(200);
      const body = await claim.json() as { resolved_image: ImageReference; image_source: string };
      expect(body.image_source, 'source_systems.default_image wins over team default').toBe('source_system_default');
      expect(body.resolved_image, 'resolved_image is the source-system default object').toEqual(sourceSystemDefaultImage);
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.76 equal image strings across teams grant no cross-team authority', async () => {
  ...
  try {
    ...
    sourceSystemBId = (await sourceB.json() as { source_system_id: string }).source_system_id;
  ...
  const exec = teamExecutorFor({ teamId: teamAId, executorId: uniqueExecutorId() });
  await registerExecutor(exec, worker.baseUrl, {
    scope: 'team',
    team_id: teamAId,
    executor_type: 'executor_docker_opehands',
  ...
  await test.step('team-a Executor attempting to claim a team-b task returns non-revealing 404 even though default_image objects are structurally equal', async () => {
    const api = await exec.api(worker.baseUrl);
    try {
      const resp = await api.post(`/v1/executors/${encodeURIComponent(exec.attach()['X-FlowAI-Executor-Id'] ?? '')}/claim`, {
        data: { task_id: teamBTask.task_id, command_id: 'C-1' },
      });
      expect(resp.status(), 'equal image objects grant no cross-team authority').toBe(404);
    } finally {
      await api.dispose();
    }
  });
