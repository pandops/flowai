...
test('v0002.3 executor discovers only pending same-team tasks matching the registered exact tag', async () => {
  ...
  await test.step('GET /v1/executors/{id}/tasks?tag=openhands returns 200 with only the same-team openhands pending summary', async () => {
    ...
    try {
      const resp = await api.get(`/v1/executors/${encodeURIComponent(execA.attach()['X-FlowAI-Executor-Id'] ?? '')}/tasks?tag=openhands`);
      expect(resp.status()).toBe(200);
      const body = await resp.json();
      const items = readItems(body);
      expect(items.length, 'discovery returns at least one team-a openhands pending summary').toBeGreaterThanOrEqual(1);
      const ids = items.map((it) => it.task_id);
      expect(ids, 'team-a openhands task is visible to the team-a Executor').toContain(openTask.task_id);
      expect(ids, 'foreign team k8s task is NOT visible to the team-a openhands Executor').not.toContain(k8sTask.task_id);
      for (const it of items) {
        expect(it.team_id, 'every summary belongs to team-a only').toBe(bsA.admin.team_id);
        expect(it.required_tag, 'every summary carries the registered openhands tag').toBe('openhands');
        expect(it.current_state, 'every summary is projected current_state = pending').toBe('pending');
      }
      // Exact discovery summary allowlist per OpenAPI TaskSummary schema
      const firstItem = items[0];
      if (!firstItem) {
        throw new Error('discovery returned no items; positive non-vacuous check requires at least one summary');
      }
      const summaryKeys = Object.keys(firstItem as unknown as Record<string, unknown>).sort();
      expect(summaryKeys, 'TaskSummary shape is EXACTLY task_id, team_id, required_tag, current_state, ingested_at').toEqual([
        'current_state',
        'ingested_at',
        'required_tag',
        'task_id',
  ...
  await test.step('negative control: GET /v1/executors/{id}/tasks?tag=k8s is rejected (registered tag is openhands)', async () => {
    const api = await execA.api(worker.baseUrl);
    try {
      const resp = await api.get(`/v1/executors/${encodeURIComponent(execA.attach()['X-FlowAI-Executor-Id'] ?? '')}/tasks?tag=k8s`);
...
test('v0002.27 registration rejects zero-tag and multi-tag bodies with invalid_tag_count', async () => {
  ...
  await test.step('zero-tag registration is rejected with 400 invalid_tag_count', async () => {
    ...
    try {
      const resp = await api.put(`/v1/executors/${encodeURIComponent(exec.attach()['X-FlowAI-Executor-Id'] ?? '')}`, {
        data: {
          scope: 'team',
          team_id: bsA.admin.team_id,
          executor_type: 'executor_docker_opehands',
      ...
      expect(resp.status(), 'zero-tag registration is rejected with 400 invalid_tag_count').toBe(400);
    } finally {
      await api.dispose();
    }
  });
  ...
  await test.step('multi-tag registration is rejected with 400 invalid_tag_count', async () => {
    ...
    try {
      ...
      expect(resp.status(), 'multi-tag registration is rejected with 400 invalid_tag_count').toBe(400);
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.28 capacity observations are informational; claim is not gated on saturated capacity', async () => {
  ...
  await test.step('Executor record persists the saturated observation', async () => {
    const api = await exec.api(worker.baseUrl);
    try {
      const resp = await api.get(`/v1/executors/${encodeURIComponent(exec.attach()['X-FlowAI-Executor-Id'] ?? '')}`);
      expect(resp.status()).toBe(200);
      const body = await resp.json();
...
test('v0002.34 discovery isolates team before pagination; foreign point probe returns non-revealing 404', async () => {
  ...
  await test.step('team-a Executor discovery returns 200 with ONLY team-a summaries; team-b is invisible', async () => {
    ...
    try {
      ...
      const body = await resp.json();
      const items = readItems(body);
      expect(items.length, 'team-a discovery is not empty (positive non-vacuous check)').toBeGreaterThanOrEqual(1);
      const ids = items.map((it) => it.task_id);
      expect(ids, 'team-a task is visible to team-a Executor (positive non-vacuous check)').toContain(teamATask.task_id);
      expect(ids, 'foreign team-b summary is not visible to team-a Executor').not.toContain(teamBTask.task_id);
      for (const it of items) {
        expect(it.team_id, 'every summary belongs to team-a only').toBe(bsA.admin.team_id);
        expect(it.required_tag, 'every summary carries the registered openhands tag').toBe('openhands');
      }
  ...
  await test.step('negative control: team-a Executor point probe for team-b task returns non-revealing 404 (same as unknown)', async () => {
    ...
    try {
      const foreign = await gwApi.get(`/v1/tasks/${encodeURIComponent(teamBTask.task_id)}`);
      const unknown = await gwApi.get(`/v1/tasks/00000000-0000-0000-0000-000000000000`);
      expect(foreign.status(), 'foreign task probe is non-revealing 404').toBe(404);
      expect(unknown.status(), 'unknown task probe is non-revealing 404').toBe(404);
...
test('v0002.54 system-owned Executor registers with scope=system, team_id=null; non-null team_id is rejected', async () => {
  ...
  await test.step('negative control: scope=system with non-null team_id is rejected with 400 without persistence', async () => {
    const api = await exec.api(worker.baseUrl);
    try {
      const resp = await api.put(`/v1/executors/${encodeURIComponent(execId)}`, {
        data: {
          scope: 'system',
          team_id: 'team-foreign',
          executor_type: 'executor_docker_opehands',
          identity: 'exec-v0002-54-system-foreign',
          authorized_tag: 'openhands',
          max_capacity: 1,
          running_count: 0,
          runtime_metadata: {},
      ...
      expect(resp.status(), 'system scope with non-null team_id is rejected with 400').toBe(400);
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.55 system-owned Executor discovers pending tasks across teams sharing its registered tag', async () => {
  ...
  await test.step('GET /v1/executors/{id}/tasks?tag=openhands returns 200 with BOTH teams\' summaries', async () => {
    ...
    try {
      const resp = await api.get(`/v1/executors/${encodeURIComponent(sysExec.attach()['X-FlowAI-Executor-Id'] ?? '')}/tasks?tag=openhands`);
      expect(resp.status()).toBe(200);
      const body = await resp.json();
      const items = readItems(body);
      expect(items.length, 'system discovery returns at least two team summaries (positive non-vacuous check)').toBeGreaterThanOrEqual(2);
      const ids = items.map((it) => it.task_id);
      expect(ids, 'team-a task is visible to system Executor (positive non-vacuous check)').toContain(teamATask.task_id);
      expect(ids, 'team-b task is visible to system Executor (positive non-vacuous check)').toContain(teamBTask.task_id);
      const teamIds = items.map((it) => it.team_id);
      expect(teamIds, 'team-a summary is visible to system Executor').toContain(bsA.admin.team_id);
      expect(teamIds, 'team-b summary is visible to system Executor').toContain(bsB.admin.team_id);
      for (const it of items) {
        expect(it.required_tag, 'every summary carries the registered openhands tag').toBe('openhands');
        expect(it.current_state, 'every summary is projected current_state = pending').toBe('pending');
      }
...
test('v0002.56 team-owned Executor discovery remains team-scoped even when a system Executor shares the tag', async () => {
  ...
  await test.step('team-a Executor never sees team-b summaries even with a system Executor sharing the tag', async () => {
    ...
    try {
      const resp = await api.get(`/v1/executors/${encodeURIComponent(teamExec.attach()['X-FlowAI-Executor-Id'] ?? '')}/tasks?tag=openhands`);
      expect(resp.status()).toBe(200);
      const body = await resp.json();
      const items = readItems(body);
      expect(items.length, 'team-a discovery is not empty (positive non-vacuous check)').toBeGreaterThanOrEqual(1);
      ...
      expect(ids, 'team-a task is visible to team-a Executor (positive non-vacuous check)').toContain(teamATask.task_id);
      const teamIds = items.map((it) => it.team_id);
      expect(teamIds.every((tid) => tid === bsA.admin.team_id), 'all summaries belong to team-a').toBe(true);
      expect(ids, 'team-b task never appears in team-a discovery').not.toContain(teamBTask.task_id);
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.59 re-registration with a different scope is rejected without mutation; original row remains', async () => {
  ...
  await test.step('system-owned Executor cannot be re-registered as team-owned (400 without mutation)', async () => {
    ...
    try {
      ...
      expect(resp.status(), 'scope-change re-registration is rejected with 400').toBe(400);
      ...
      expect(body.team_id, 'team_id remains null after rejected re-registration').toBeNull();
    } finally {
      await api.dispose();
    }
  });
  ...
  await test.step('team-owned Executor cannot be re-registered as system-owned (400 without mutation)', async () => {
    ...
    const api = await teamExec.api(worker.baseUrl);
    try {
      const resp = await api.put(`/v1/executors/${encodeURIComponent(teamId)}`, {
        data: {
          scope: 'system',
          team_id: null,
          executor_type: 'executor_docker_opehands',
      ...
      expect(resp.status(), 'team-owned Executor cannot switch to system scope (400)').toBe(400);
...
test('v0002.67 FIFO discovery orders eligible pending tasks by (ingested_at ASC, task_id ASC)', async () => {
  ...
  const ingested = [] as Array<{ sourceId: string; task_id: string; ingested_at: string }>;
  for (const entry of ingest) {
    ...
    ingested.push({ sourceId: entry.sourceId, task_id: task.task_id, ingested_at: task.ingested_at });
  }
  ...
  await test.step('discovery returns summaries in (ingested_at ASC, task_id ASC) order with team-a eligibility', async () => {
    ...
    try {
      ...
      const body = await resp.json();
      const items = readItems(body);
      expect(items.length, 'discovery returns at least four team-a openhands pending summaries').toBeGreaterThanOrEqual(4);
      for (const it of items) {
        expect(it.team_id, 'eligibility predicate restricts the page to team-a').toBe(bsA.admin.team_id);
        expect(it.required_tag, 'every summary carries the registered openhands tag').toBe('openhands');
        expect(it.current_state, 'every summary is projected current_state = pending').toBe('pending');
      }
      const expectedOrder = [...ingested].sort((a, b) => {
      ...
      for (const observedIndex of observedIndices) {
        expect(observedIndex, 'every ingested task is visible in the discovery page').toBeGreaterThanOrEqual(0);
      }
      for (let i = 1; i < observedIndices.length; i += 1) {
        const prev = observedIndices[i - 1];
        const curr = observedIndices[i];
        if (prev === undefined || curr === undefined) {
          throw new Error('observed index is unexpectedly undefined while comparing FIFO order');
        }
        expect(curr, 'discovery order matches expected FIFO (ingested_at ASC, task_id ASC) order').toBeGreaterThan(prev);
      }
    } finally {
      await api.dispose();
    }
