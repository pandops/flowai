...
//      the SAN hostname set. Any readiness failure identifies the
//      production HTTPS/mTLS contract that did not initialize.
...
//   5. Verifies the Postgres container and Registry client enforce
//      server-chain verification: the test never claims plain TCP
//      proves TLS and names the relevant configuration on failure.
...
test("v0002.20 protected APIs reject untrusted transport identities before request processing; mTLS, WSS, and Postgres-chain contract", async () => {
  ...
  await test.step("ephemeral TLS material is generated; private keys live only in the temp directory", async () => {
    ...
    expect(
      existsSync(material.caKeyPath),
      "CA key exists (test does not log it)",
    ).toBe(true);
    expect(existsSync(material.serverCertPath), "server cert exists").toBe(
      true,
    );
    expect(
      existsSync(material.serverKeyPath),
      "server key exists (test does not log it)",
    ).toBe(true);
    expect(
      existsSync(material.postgresServerCertPath),
      "postgres server cert exists",
    ).toBe(true);
    expect(
      existsSync(material.postgresServerKeyPath),
      "postgres server key exists (test does not log it)",
    ).toBe(true);
    for (const c of [
      material.trustedAdmin,
      material.trustedListener,
      material.trustedTeamExecutor,
      material.trustedSystemExecutor,
      ...
      expect(
        existsSync(c.keyPath),
        `client key ${c.cn} exists (test does not log it)`,
      ).toBe(true);
    }
  });
  ...
  await test.step("start TLS-mode Registry worker; readiness probe requires the trusted HTTPS+mTLS listener", async () => {
    try {
      ...
      const diagnostics =
        (primary as { diagnostics?: string }).diagnostics ?? "";
      const baseMsg =
        (primary as { message?: string }).message ?? String(primary);
      throw new Error(
        "v0002.20 production HTTPS+mTLS startup failed: " +
          "startRegistryWorker did not reach TLS-mode readiness on https://" +
          ...
          "The State Registry must bind an https:// listener on the requested port, " +
  ...
  await test.step("normal untagged binary serves production admin and system-Executor routes through mTLS identities", async () => {
    const adminCreate = await httpsRequest(`${baseHttps}/admin/teams`, {
      method: "POST",
      clientCertPath: material.trustedAdmin.certPath,
      clientKeyPath: material.trustedAdmin.keyPath,
      caBundlePath: material.caCertPath,
      servername: LoopbackServername,
      headers: {
        "content-type": "application/json",
        "x-request-id": `v0002-20-admin-${Date.now().toString(36)}`,
      },
      body: JSON.stringify({
        team_name: `v0002-20-production-${Date.now().toString(36)}`,
        default_image: imageReference("default-v0002-20-production"),
      }),
    });
    expect(
      adminCreate.status,
      `trusted admin cert creates through production route; body=${adminCreate.body.slice(0, 256)}`,
    ).toBe(201);

    const executorID = material.trustedSystemExecutor.cn;
    const executorRegister = await httpsRequest(
      `${baseHttps}/v1/executors/${encodeURIComponent(executorID)}`,
      {
        method: "PUT",
        clientCertPath: material.trustedSystemExecutor.certPath,
        clientKeyPath: material.trustedSystemExecutor.keyPath,
        caBundlePath: material.caCertPath,
        servername: LoopbackServername,
        headers: {
          "content-type": "application/json",
          "x-request-id": `v0002-20-executor-${Date.now().toString(36)}`,
        },
        body: JSON.stringify({
          scope: "system",
          team_id: null,
          ...
          max_capacity: 1,
          running_count: 0,
          runtime_metadata: { transport_test: true },
        }),
      },
    );
    expect(
      [200, 201],
      `trusted system Executor cert registers through production route; status=${executorRegister.status} body=${executorRegister.body.slice(0, 256)}`,
    ).toContain(executorRegister.status);
  });
  ...
  await test.step("trusted service cert completes TLS; /v1/livez and /v1/readyz answer 200 with the verified peer CN", async () => {
    ...
    expect(
      livez.status,
      "trusted Gateway cert reaches /v1/livez over mTLS",
    ).toBe(200);
    expect(
      livez.peerCn,
      "client verifies the Registry server certificate CN",
    ).toBe("flowai-v0002-20-server");
    ...
    expect(
      readyz.status,
      "trusted Gateway cert reaches /v1/readyz over mTLS",
    ).toBe(200);
  });
  ...
  await test.step("handshake fails before any HTTP read: no client cert", async () => {
    ...
    expect(
      probe.ok,
      "TLS handshake without a client cert is rejected before HTTP",
    ).toBe(false);
    expect(
  ...
  await test.step("handshake fails before any HTTP read: expired client cert", async () => {
    ...
    expect(
      probe.ok,
      "TLS handshake with an expired client cert is rejected before HTTP",
    ).toBe(false);
    expect(
      probe.tlsErrorCode === "CERT_HAS_EXPIRED" ||
        /expired/i.test(probe.message),
  ...
  await test.step("handshake fails before any HTTP read: untrusted-CA client cert", async () => {
    ...
    expect(
      probe.ok,
      "TLS handshake with a cert from an untrusted CA is rejected before HTTP",
    ).toBe(false);
    expect(
      probe.tlsErrorCode === "UNABLE_TO_VERIFY_LEAF_SIGNATURE" ||
        probe.tlsErrorCode === "CERT_UNTRUSTED" ||
        probe.tlsErrorCode === "ERR_SSL_TLSV1_ALERT_UNKNOWN_CA" ||
        /unable to verify/i.test(probe.message) ||
        /untrusted|unknown ca/i.test(probe.message),
  ...
  await test.step("server hostname verification is active: wrong servername is rejected before HTTP", async () => {
    ...
    expect(
      probe.ok,
      "TLS handshake with a wrong servername is rejected before HTTP",
    ).toBe(false);
    expect(
  ...
  await test.step("valid wrong-role cert completes TLS but the strict peerauth parser rejects its unknown OU role (no trusted headers)", async () => {
    // wrong-role OU is not in the known role set, so the parser
    // drops every X-FlowAI-* identity header and protected
    // endpoints see an unauthenticated request (401, NOT 403).
    const livez = await httpsRequest(`${baseHttps}/v1/livez`, {
    ...
    expect(
      livez.status,
      "public health probe remains reachable after the trusted TLS handshake",
    ).toBe(200);
    ...
    expect(
      adminPost.status,
      "wrong-role cert has no trusted headers; admin handler returns 401",
    ).toBe(401);
  });
  ...
  await test.step("listener, Executor, and Gateway certs each cannot call /admin/*; only a system-administrator identity may", async () => {
    ...
    for (const c of adminCases) {
      ...
      expect(resp.status, `${c.label} cert is forbidden on /admin/teams`).toBe(
        403,
      );
    }
  });

  await test.step("trusted listener cert (real listener subject with source-system serial) reaches the listener authorization boundary", async () => {
    // trusted listener subject: OU=listener, O=team-a,
    // serialNumber=source-a. Parser promotes it to
    // X-FlowAI-Role=listener + X-FlowAI-Team-Id=team-a +
    // X-FlowAI-Source-System-Id=source-a. The listener
    // adapter accepts the body format and reaches its
    // binding check; precise status depends on whether team-a
    // has a registered source-system in this test's
    // postgres fixture (none), but it MUST NOT be 401.
    const resp = await httpsRequest(`${baseHttps}/v1/tasks`, {
      method: "POST",
      clientCertPath: material.trustedListener.certPath,
      clientKeyPath: material.trustedListener.keyPath,
      caBundlePath: material.caCertPath,
      servername: LoopbackServername,
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        team_id: "team-a",
        source_system_id: "source-a",
        source_id: `v0002-20-${Date.now().toString(36)}`,
        task_type_id: "task-type-a",
        payload: { hello: "world" },
      }),
    });
    expect(
      resp.status !== 401 && resp.status !== 404 && resp.status !== 405,
      `trusted listener cert must reach the listener adapter (NOT 401, NOT 404, NOT 405); got status=${resp.status} body=${resp.body.slice(0, 256)}`,
    ).toBe(true);
  });

  await test.step("valid same-role wrong-team listener cert is rejected at the team authorization boundary without mutation", async () => {
    // wrongTeamCert is a real listener (OU=listener, O=team-b,
    // serialNumber=source-b). The parser keeps every header
    // and the listener authorization boundary rejects the
    // request because the cert's team-b binding doesn't
    // match the submitted team-a body.
    const resp = await httpsRequest(`${baseHttps}/v1/tasks`, {
      method: "POST",
      ...
      body: JSON.stringify({
        team_id: "team-a",
        source_system_id: "source-a",
        source_id: `v0002-20-wrongteam-${Date.now().toString(36)}`,
        task_type_id: "task-type-a",
        payload: { hello: "world" },
      }),
    });
    expect(
      resp.status,
      "same-role wrong-team listener cert is forbidden at the team boundary before mutation",
    ).toBe(403);
  });
  ...
  await test.step("WebSocket upgrade uses wss:// with a trusted Gateway cert; unauthenticated/untrusted handshakes fail", async () => {
    ...
    const noCertResult = await new Promise<{ ok: boolean; message: string }>(
      (resolve) => {
        let settled = false;
    ...
    expect(
      noCertResult.ok === false &&
        /certificate required|alert/i.test(noCertResult.message),
    ...
    const untrustedResult = await new Promise<{ ok: boolean; message: string }>(
      (resolve) => {
        let settled = false;
        const onError = (err: Error): void => {
          if (!settled) {
    ...
    expect(
      untrustedResult.ok === false &&
        /unable to verify|untrusted|unknown ca/i.test(untrustedResult.message),
  ...
  await test.step("Postgres server-chain verification: valid chain reaches /v1/readyz=200; unverifiable chain fails closed before ready (precise RED when not implemented)", async () => {
    ...
    try {
      pgTlsWorker = await startRegistryWorker({
        ...
        tlsClientCaPath: material.caCertPath,
        tlsRequireClientCert: true,
        // Stage the postgres server cert + key (signed by the
        // trusted CA, EKU serverAuth, SAN DNS:localhost +
        // IP:127.0.0.1) into the ephemeral postgres:16
        // container so the State Registry's Go client can
        // complete a verify-full chain handshake against it.
        tlsPostgresServerCertPath: material.postgresServerCertPath,
        tlsPostgresServerKeyPath: material.postgresServerKeyPath,
        // The Go client trusts the same trusted CA bundle that
        // signed the postgres server cert. With verify-full, the
        // chain validates and the worker reaches /v1/readyz=200.
        tlsPostgresCaPath: material.caCertPath,
        tlsPostgresVerifyMode: "verify-full",
        tlsClientCertPath: material.trustedGateway.certPath,
        tlsClientKeyPath: material.trustedGateway.keyPath,
      ...
      const result: HttpsRequestResult = await httpsRequest(
        `${pgTlsWorker.baseUrl}/v1/readyz`,
        {
          clientCertPath: material.trustedGateway.certPath,
          clientKeyPath: material.trustedGateway.keyPath,
          caBundlePath: material.caCertPath,
      ...
      expect(
        result.status,
        "with a verified Postgres chain, /v1/readyz returns 200",
      ).toBe(200);
    } catch (primary) {
      const baseMsg =
        (primary as { message?: string }).message ?? String(primary);
      throw new Error(
        "v0002.20 RED (State Registry does not yet verify the Postgres server chain): " +
          ...
          "when the chain is unverifiable. Underlying error: " +
          baseMsg,
      );
    } finally {
      if (pgTlsWorker) {
  ...
  await test.step("Postgres server-chain verification: invalid CA/hostname configuration fails closed before ready", async () => {
    ...
    try {
      pgTlsWorker = await startRegistryWorker({
        ...
        tlsClientCaPath: material.caCertPath,
        tlsRequireClientCert: true,
        // The postgres container STILL presents the trusted-CA
        // signed cert (the server chain is verifiable), but the
        // Go client is configured to trust an UNRELATED CA
        // bundle. The mismatch is the precise trigger for a
        // verify-full failure; the contract is fail-closed.
        tlsPostgresServerCertPath: material.postgresServerCertPath,
        tlsPostgresServerKeyPath: material.postgresServerKeyPath,
    ...
    if (!observedFailure) {
      throw new Error(
        "v0002.20 RED (State Registry does not fail closed on an unverifiable Postgres chain): " +
          ...
          "and STATE_REGISTRY_POSTGRES_TLS_MODE=verify-full even though the Postgres server certificate is signed " +
          "by a different CA. The Registry must refuse readiness when the configured chain is unverifiable. " +
  ...
  await test.step("AES-256-GCM data key fail-closed: omitting STATE_REGISTRY_AES_KEY_HEX must refuse to start the worker", async () => {
    ...
    try {
      ...
      const diagnostics =
        (primary as { diagnostics?: string }).diagnostics ?? "";
      expect(
        /AES_KEY_HEX|aes|key/i.test(`${msg}; ${diagnostics}`),
        `AES-key fail-closed: startup failure names the AES key as the cause: ${msg}; diagnostics_tail=${diagnostics.slice(-512)}`,
      ).toBe(true);
    } finally {
      if (w) {
        try {
