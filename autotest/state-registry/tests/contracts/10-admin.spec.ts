...
import { ingestPendingTask, imageReference, type ImageReference } from './_setup';
...
async function tableCount(table: 'teams' | 'source_systems' | 'task_types'): Promise<number> {
...
test('v0002.60 admin creates a team with REQUIRED default_image; duplicate team_name and missing fields are rejected', async () => {
  ...
  try {
    await test.step('positive control: POST /admin/teams returns 201 with REQUIRED default_image and unique team_name', async () => {
      ...
      const body = await resp.json() as { team_id: string; team_name: string; default_image: ImageReference };
      teamId = body.team_id;
      ...
      expect(body.default_image, 'default_image is REQUIRED and persisted as ImageReference').toEqual(expectedImage);
    });
    await test.step('negative control: duplicate team_name is rejected with 400', async () => {
      const resp = await adminApi.post('/admin/teams', {
        data: { team_name: `${suffix}-a`, default_image: imageReference(`default-${suffix}-duplicate`) },
      });
      expect(resp.status(), 'duplicate team_name is rejected without replacement').toBe(400);
    });
    await test.step('negative control: missing default_image is rejected with 400', async () => {
      const resp = await adminApi.post('/admin/teams', {
        data: { team_name: `${suffix}-noimage` },
      });
      expect(resp.status(), 'missing default_image is rejected without persistence').toBe(400);
    });
    await test.step('negative control: missing team_name is rejected with 400', async () => {
      ...
      expect(resp.status(), 'missing team_name is rejected without persistence').toBe(400);
    });
  ...
  await test.step('the canonical team row is durable across Registry restart', async () => {
    ...
    expect(persisted).toBe(`${suffix}-a|${expectedImage.repository}|${expectedImage.digest}`);
  });
});
  ...
  try {
    await test.step('positive control: POST /admin/teams creates team-a and team-b', async () => {
      const teamA = await adminApi.post('/admin/teams', {
        data: { team_name: `${suffix}-a`, default_image: imageReference(`default-${suffix}-a`) },
      });
      ...
      const teamB = await adminApi.post('/admin/teams', {
        data: { team_name: `${suffix}-b`, default_image: imageReference(`default-${suffix}-b`) },
      });
    ...
    await test.step('positive control: POST /admin/source-systems returns 201 for the first team', async () => {
      const resp = await adminApi.post('/admin/source-systems', {
        data: { team_id: teamAId, listener_identity: listenerIdentity, default_image: sourceImage },
      });
      ...
      expect(sourceSystemId, 'source system has an immutable server-generated id').toBeTruthy();
    ...
    await test.step('negative control: same listener_identity under the SAME team is rejected with 409', async () => {
      ...
      expect(resp.status(), 'duplicate listener_identity under the same team is rejected').toBe(409);
    });
    await test.step('negative control: same listener_identity under a DIFFERENT team is rejected with 409', async () => {
      ...
      expect(resp.status(), 'duplicate listener_identity across teams is rejected').toBe(409);
    });
    await test.step('negative control: source-system referencing a non-existent team is rejected with 400', async () => {
      ...
      expect(resp.status(), 'non-existent team reference is rejected').toBe(400);
    });
    await test.step('negative control: unauthenticated caller is rejected with 401', async () => {
      const resp = await fetch(`${worker.baseUrl}/admin/source-systems`, {
        ...
        body: JSON.stringify({ team_id: teamAId, listener_identity: `${suffix}-anonymous` }),
      });
      expect(resp.status).toBe(401);
    });
    await test.step('negative control: source-system reattachment has no update route', async () => {
      const resp = await adminApi.put(`/admin/source-systems/${encodeURIComponent(sourceSystemId)}`, {
  ...
  try {
    ...
    await test.step('positive control: POST /admin/task-types returns 201 with REQUIRED execution_tag', async () => {
      const resp = await adminApi.post('/admin/task-types', {
        data: { team_id: teamId, execution_tag: 'openhands', default_image: taskTypeImage },
      });
      ...
      expect(body.task_type_id, 'task-type create returns an immutable task_type_id').toBeTruthy();
...
test('v0002.63 Executor registration never creates or updates a team; only POST /admin/teams creates one', async () => {
  ...
  try {
    await test.step('negative control: PUT /v1/executors/{id} with scope=team and team_id=team-ghost is rejected with 400 without persistence', async () => {
      const executor = teamExecutorFor({ teamId: 'team-ghost', executorId: uniqueExecutorId() });
      const api = await executor.api(worker.baseUrl);
      try {
        const resp = await api.put(`/v1/executors/${encodeURIComponent(executor.attach()['X-FlowAI-Executor-Id'] ?? '')}`, {
          data: {
            scope: 'team',
            team_id: 'team-ghost',
            executor_type: 'executor_docker_opehands',
            identity: 'exec-v0002-63-ghost',
        ...
        expect(resp.status(), 'Executor registration referencing a non-existent team is rejected without persistence').toBe(400);
      } finally {
        await api.dispose();
      }
    });
    await test.step('positive control: POST /admin/teams creates the team (only admin path can)', async () => {
      const resp = await adminApi.post('/admin/teams', {
        data: { team_name: `v0002-63-${Date.now().toString(36)}-real`, default_image: imageReference('default-v0002-63') },
      });
      expect(resp.status(), 'admin team create returns 201').toBe(201);
    ...
    await test.step('known team passes the Section 3 guard and reaches the explicit registration boundary', async () => {
      ...
      try {
        ...
        const resp = await api.put(`/v1/executors/${encodeURIComponent(executorId)}`, {
          data: {
            scope: 'team',
            team_id: realTeamId,
            executor_type: 'executor_docker_opehands',
            identity: 'exec-v0002-63-real',
            authorized_tag: 'openhands',
            max_capacity: 1,
            running_count: 0,
            runtime_metadata: {},
      ...
      try {
        ...
        const resp = await api.put(`/v1/executors/${encodeURIComponent(executorId)}`, {
          data: {
            ...
            team_id: realTeamId,
            executor_type: 'executor_docker_opehands',
            identity: 'exec-v0002-63-system-invalid',
            authorized_tag: 'openhands',
            max_capacity: 1,
            running_count: 0,
            runtime_metadata: {},
...
test('v0002.79 admin lists configured tags with the documented exact allowlist (team_id, task_type_id, execution_tag)', async () => {
  ...
  try {
    await test.step('positive control: POST /admin/teams + POST /admin/task-types seeds the admin tag projection', async () => {
      ...
      sourceSystemId = (await source.json() as { source_system_id: string }).source_system_id;
  ...
  await test.step('positive control: GET /admin/tags returns 200 with the documented 3-field exact allowlist', async () => {
    ...
    try {
      ...
      expect(resp.status(), 'system administrator reads /admin/tags with 200').toBe(200);
      const body = await resp.json();
      expect(Array.isArray(body.items)).toBe(true);
      for (const entry of body.items as Array<Record<string, unknown>>) {
        const keys = Object.keys(entry).sort();
        expect(keys, 'each admin tag entry has EXACTLY team_id, task_type_id, execution_tag').toEqual(['execution_tag', 'task_type_id', 'team_id']);
      }
    } finally {
  ...
  await test.step('negative control: GET /admin/tags as Gateway identity is rejected with 403', async () => {
    const api = await gatewayFor({ teamId, operatorId: `op-${teamId}` }).api(worker.baseUrl);
    try {
      const resp = await api.get('/admin/tags?limit=200');
      expect(resp.status(), 'Gateway identity cannot read /admin/tags').toBe(403);
    } finally {
      await api.dispose();
    }
  });
  await test.step('negative control: GET /admin/tags as listener identity is rejected with 403', async () => {
    const api = await listenerFor({ teamId, listenerIdentity, sourceSystemId }).api(worker.baseUrl);
    try {
      const resp = await api.get('/admin/tags?limit=200');
      expect(resp.status(), 'listener identity cannot read /admin/tags').toBe(403);
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.80 admin lists all tasks across teams with the documented exact 11-field allowlist', async () => {
  ...
  await test.step('positive control: GET /admin/tasks returns 200 with EXACTLY the documented 11-field shape per entry', async () => {
    ...
    try {
      ...
      expect(resp.status(), 'system administrator reads /admin/tasks with 200').toBe(200);
      const body = await resp.json();
      expect(Array.isArray(body.items)).toBe(true);
      const expectedKeys = [
        'claimed_at', 'current_state', 'executor_id', 'ingested_at', 'owner_command_id',
        'required_tag', 'source_id', 'source_system_id', 'task_id', 'task_type_id', 'team_id',
      ];
      for (const entry of body.items as Array<Record<string, unknown>>) {
        const keys = Object.keys(entry).sort();
        expect(keys, 'admin task entry has EXACTLY 11 documented fields').toEqual([...expectedKeys].sort());
      }
    } finally {
      await adminRead.dispose();
    }
...
test('v0002.81 admin reads enforce identity and are non-mutating; non-admin identities are rejected before any row is read', async () => {
  ...
  await test.step('negative control: listener / Executor / Gateway are each rejected with 403 before any row is read', async () => {
    const cases = [
      { name: 'listener', ctx: listenerFor({ teamId, listenerIdentity, sourceSystemId }) },
      { name: 'team Executor', ctx: teamExecutorFor({ teamId, executorId: uniqueExecutorId() }) },
      { name: 'trusted Gateway', ctx: gatewayFor({ teamId, operatorId: `op-${teamId}` }) },
    ];
    for (const c of cases) {
      ...
      try {
        ...
        expect(tagsResp.status(), `${c.name} is rejected on /admin/tags with 403`).toBe(403);
        expect(tasksResp.status(), `${c.name} is rejected on /admin/tasks with 403`).toBe(403);
      } finally {
        await api.dispose();
      }
    }
  ...
  await test.step('negative control: unauthenticated caller cannot reach /admin/tags or /admin/tasks (401)', async () => {
    ...
    expect(tagsResp.status, 'unauthenticated /admin/tags is rejected with 401').toBe(401);
    expect(tasksResp.status, 'unauthenticated /admin/tasks is rejected with 401').toBe(401);
  });
  ...
  await test.step('positive control: system administrator can read both admin projections with 200', async () => {
    ...
    try {
      ...
      expect(tagsResp.status(), 'system administrator reads /admin/tags with 200').toBe(200);
      expect(tasksResp.status(), 'system administrator reads /admin/tasks with 200').toBe(200);
    } finally {
      await adminRead.dispose();
    }
  });
