...
test('v0002.7 gateway-mediated control is team-scoped and audit-backed; direct listener copy is rejected', async () => {
  ...
  try {
    await test.step('trusted Gateway creates the cancel control with 202', async () => {
      const resp = await gwApi.post(`/v1/tasks/${encodeURIComponent(task.task_id)}/controls`, {
        data: { action: 'cancel', idempotency_key: 'idem-v0002-7-1', reason: 'unit-test' },
      });
      expect(resp.status(), 'trusted Gateway creates the team-a cancel control').toBe(202);
      const body = await resp.json();
      expect(body.team_id, 'control carries the verified team_id').toBe(bsA.admin.team_id);
      expect(body.task_id, 'control references the team task_id').toBe(task.task_id);
    });
  ...
  await test.step('negative control: direct listener copy is rejected with 403 without mutation', async () => {
    ...
    try {
      const resp = await listenerApi.post(`/v1/tasks/${encodeURIComponent(task.task_id)}/controls`, {
        data: { action: 'cancel', idempotency_key: 'idem-v0002-7-direct' },
      });
      expect(resp.status(), 'listener identity cannot create Gateway-only controls').toBe(403);
    } finally {
...
test('v0002.12 service-identity enforcement: impersonating and unauthenticated calls are rejected; admin-only paths reject non-admin identities', async () => {
  ...
  await test.step('listener impersonation for a foreign team is rejected with 403', async () => {
    ...
    try {
      ...
      expect(resp.status(), 'listener cannot ingest for a foreign team').toBe(403);
    } finally {
      await api.dispose();
    }
  });
  await test.step('admin endpoint rejects trusted Gateway identity with 403', async () => {
    ...
    try {
      const resp = await api.post('/admin/teams', {
        data: { team_name: 'gateway-impersonator', default_image: imageReference('default-gateway') },
      });
      expect(resp.status(), 'trusted Gateway identity is rejected on the admin transport boundary').toBe(403);
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.37 control creation is team-scoped and idempotent; foreign task control returns non-revealing 404', async () => {
  ...
  try {
    await test.step('team-a Gateway creates a control for the team-a task; retry is idempotent', async () => {
      const first = await gwApi.post(`/v1/tasks/${encodeURIComponent(taskA.task_id)}/controls`, {
        data: { action: 'cancel', idempotency_key: 'v0002-37-idem-1' },
      });
      expect(first.status(), 'team-a Gateway creates the team-a control').toBe(202);
      const retry = await gwApi.post(`/v1/tasks/${encodeURIComponent(taskA.task_id)}/controls`, {
        data: { action: 'cancel', idempotency_key: 'v0002-37-idem-1' },
      });
    ...
    await test.step('team-a Gateway attempting a control for the team-b task returns non-revealing 404', async () => {
      const resp = await gwApi.post(`/v1/tasks/${encodeURIComponent(taskB.task_id)}/controls`, {
        data: { action: 'cancel', idempotency_key: 'v0002-37-foreign' },
      });
      expect(resp.status(), 'foreign-task control is non-revealing 404').toBe(404);
    });
  } finally {
    await gwApi.dispose();
  }
...
test('v0002.41 team-filter precedes pagination, counts, and audit reads', async () => {
  ...
  await test.step('team-a Gateway list returns ONLY team-a tasks with 200', async () => {
    ...
    try {
      ...
      expect(resp.status(), 'trusted Gateway list returns 200 for the authenticated team').toBe(200);
      const body = await resp.json();
      const items = Array.isArray(body.items) ? body.items : [];
      const teamIds = (items as Array<{ team_id: string }>).map((it) => it.team_id);
      const taskIds = (items as Array<{ task_id: string }>).map((it) => it.task_id);
      expect(items.length, 'team-a list is non-empty and proves own-team rows exist').toBeGreaterThan(0);
      expect(taskIds, 'team-a list contains the team-a task_id').toContain(taskA.task_id);
      expect(teamIds.every((tid) => tid === bsA.admin.team_id), 'all items belong to team-a').toBe(true);
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.42 WebSocket upgrade binds at upgrade and never emits cross-team frames', async () => {
  ...
  const execB = teamExecutorFor({ teamId: bsB.admin.team_id, executorId: uniqueExecutorId() });
  ...
  try {
    ...
    await test.step('each live connection receives ONLY frames for its immutable bound team_id over a bounded observation period', async () => {
      ...
      while (Date.now() < deadline) {
        ...
        while (Date.now() < slotEnd) {
          const next = await drainOneFrame(sockA, Math.max(1, slotEnd - Date.now()));
          if (next !== undefined) framesA.push(next);
        }
        const next = await drainOneFrame(sockB, 50);
        if (next !== undefined) framesB.push(next);
      }
      expect(framesA.length, 'team-a socket observed at least one Registry-appended frame during the observation window').toBeGreaterThan(0);
      expect(framesB.length, 'team-b socket observed at least one Registry-appended frame during the observation window').toBeGreaterThan(0);
      for (let i = 0; i < framesA.length; i++) {
        const frame = framesA[i];
        expect(frameTeamID(frame), `team-a frame[${i}] belongs only to team-a`).toBe(bsA.admin.team_id);
      }
      for (let i = 0; i < framesB.length; i++) {
        const frame = framesB[i];
        expect(frameTeamID(frame), `team-b frame[${i}] belongs only to team-b`).toBe(bsB.admin.team_id);
      }
    });
    ...
    await test.step('team-a filter naming a real team-b task produces no frame even after a new team-b lifecycle event', async () => {
      ...
      try {
        ...
        expect(postEvent?.status(), 'team-b running event is accepted with 202 to seed a foreign frame').toBe(202);
        await expect(filtered.nextMessage(1_000)).rejects.toThrow(/message timeout/);
      } finally {
...
async function drainOneFrame(socket: { nextMessage: (timeoutMs: number) => Promise<unknown> }, timeoutMs: number): Promise<unknown | undefined> {
  try {
...
test('v0002.43 State Registry trusts only verified Gateway context; copied headers are rejected', async () => {
  ...
  await test.step('trusted Gateway can read team-a tasks with 200 and sees the seeded own-team row', async () => {
    ...
    try {
      ...
      const teamIds = (items as Array<{ team_id: string }>).map((it) => it.team_id);
      expect(items.length, 'team-a list is non-empty and proves own-team rows exist').toBeGreaterThan(0);
      expect(taskIds, 'team-a list contains the seeded team-a task_id').toContain(taskA.task_id);
      expect(teamIds.every((tid) => tid === bsA.admin.team_id), 'all items belong to team-a').toBe(true);
    } finally {
      await api.dispose();
    }
  ...
  await test.step('different Gateway identity sees ONLY its own team rows and proves its own rows exist', async () => {
    const other = gatewayFor({ teamId: bsOther.admin.team_id, operatorId: `op-${bsOther.admin.team_id}` });
    const api = await other.api(worker.baseUrl);
    try {
      const resp = await api.get(`/v1/tasks?limit=10`);
      expect(resp.status(), 'different Gateway list returns 200 for its own team').toBe(200);
      const body = await resp.json();
      const items = Array.isArray(body.items) ? body.items : [];
      const taskIds = (items as Array<{ task_id: string }>).map((it) => it.task_id);
      const teamIds = (items as Array<{ team_id: string }>).map((it) => it.team_id);
      expect(items.length, 'other-team list is non-empty and proves other-team rows exist').toBeGreaterThan(0);
      expect(taskIds, 'other-team list contains the seeded other-team task_id').toContain(taskOther.task_id);
      expect(teamIds.every((tid) => tid === bsOther.admin.team_id), 'items belong to the other team only').toBe(true);
      expect(taskIds, 'other-team list does not leak team-a task_id').not.toContain(taskA.task_id);
    } finally {
      await api.dispose();
    }
  });
...
test('v0002.44 team_name is display-only and never grants authority', async () => {
  ...
  await test.step('verified team-a Gateway with normal display team_name reads the team-a row and never the team-b row (returns 200)', async () => {
    ...
    try {
      const resp = await api.get('/v1/tasks?limit=10');
      expect(resp.status(), 'team_name as display-only does not affect list authorization (200)').toBe(200);
      const body = await resp.json();
      const items = Array.isArray(body.items) ? body.items : [];
      const taskIds = (items as Array<{ task_id: string }>).map((it) => it.task_id);
      expect(taskIds, 'verified team-a list contains the team-a task_id').toContain(taskA.task_id);
      expect(taskIds, 'verified team-a list never leaks the team-b task_id').not.toContain(taskB.task_id);
    } finally {
      await api.dispose();
    }
  });
  await test.step('a deliberately foreign display team_name while verified team_id remains team-a cannot grant team-b authority', async () => {
    ...
    try {
      ...
      expect(listResp.status(), 'spoofed display team_name does not change list authorization (200)').toBe(200);
      const body = await listResp.json();
      const items = Array.isArray(body.items) ? body.items : [];
      const taskIds = (items as Array<{ task_id: string }>).map((it) => it.task_id);
      const teamIds = (items as Array<{ team_id: string }>).map((it) => it.team_id);
      expect(taskIds, 'spoofed display team_name still returns team-a task_id').toContain(taskA.task_id);
      expect(taskIds, 'spoofed display team_name never returns team-b task_id').not.toContain(taskB.task_id);
      expect(teamIds.every((tid) => tid === bsA.admin.team_id), 'team_id filter is anchored to verified team-a only').toBe(true);

      const controlResp = await api.post(`/v1/tasks/${encodeURIComponent(taskB.task_id)}/controls`, {
        data: { action: 'cancel', idempotency_key: 'v0002-44-foreign-name' },
      });
      expect(controlResp.status(), 'foreign display team_name does not authorize team-b control (non-revealing 404)').toBe(404);
    } finally {
      await api.dispose();
    }
  });
