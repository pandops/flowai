...
async function postTaskEvent(
  ...
  try {
    const response = await api.post(`/v1/tasks/${encodeURIComponent(taskId)}/events`, { data: body });
...
async function postSelfEvent(
  ...
  const api = await exec.api(baseUrl);
  try {
    const response = await api.post(`/v1/executors/${encodeURIComponent(executorId)}/events`, { data: body });
    return await snapshotResponse(response);
  } finally {
    await api.dispose();
  }
...
async function readClaimCreatedAt(
  ...
  try {
    ...
    if (!created) {
      throw new Error(`readClaimCreatedAt: created event missing for task=${taskId}`);
    }
...
test('v0002.5 idempotent retry of the same running event returns the original 202 and appends once', async () => {
  ...
  const claimResp = await claimAsExecutor(exec, worker.baseUrl, task.task_id, 'C-1');
  expect(claimResp.status(), 'claim returns 200').toBe(200);
  const createdAt = await readClaimCreatedAt(worker, bsA.admin.team_id, task.task_id);
  const envelope = {
  ...
  const first = await postTaskEvent(exec, worker.baseUrl, task.task_id, envelope);
  const retry = await postTaskEvent(exec, worker.baseUrl, task.task_id, envelope);
  ...
  try {
    ...
    const running = items.filter((e: { event_type: string; event_id: string }) => e.event_type === 'running' && e.event_id === '0190-running-0001');
    expect(running.length, 'running event appears exactly once').toBe(1);
    expect(items.find((e: { event_type: string }) => e.event_type === 'finished'), 'no finished event was appended').toBeUndefined();
  } finally {
    await gwApi.dispose();
  }
});
...
test('v0002.6 Executor self events are accepted only by the Executor event stream and never enter task history', async () => {
  ...
  await test.step('POST /v1/tasks/{id}/events rejects the self event shape with 403', async () => {
    const resp = await postTaskEvent(exec, worker.baseUrl, task.task_id, selfEnvelope);
    expect(resp.status(), 'self event shape is rejected on the task-event boundary').toBe(403);
  });
});
...
test('v0002.23 successful happy-path lifecycle projects [created, running, finished]', async () => {
  ...
  const r1 = await postTaskEvent(exec, worker.baseUrl, task.task_id, { ...base, event_id: '0190-running-23', event_type: 'running', occurred_at: plusOffsetMs(createdAt, 1_000), payload: { runtime: 'openhands' } });
  expect(r1.status()).toBe(202);
  const r2 = await postTaskEvent(exec, worker.baseUrl, task.task_id, { ...base, event_id: '0190-finished-23', event_type: 'finished', occurred_at: plusOffsetMs(createdAt, 61_000), payload: { status: 'ok' } });
  ...
  try {
    ...
    expect(events.status(), "ordered event history read returns 200").toBe(200);
    const body = await events.json();
    const types = (body.items as Array<{ event_type: string }>).map((e) => e.event_type);
    expect(types, 'happy path projects exactly [created, running, finished]').toEqual(['created', 'running', 'finished']);
...
test('v0002.24 failure path projects [created, running, failed] without a finished event', async () => {
  ...
  const r1 = await postTaskEvent(exec, worker.baseUrl, task.task_id, { ...base, event_id: '0190-running-24', event_type: 'running', occurred_at: plusOffsetMs(createdAt, 1_000), payload: {} });
  expect(r1.status()).toBe(202);
  const r2 = await postTaskEvent(exec, worker.baseUrl, task.task_id, { ...base, event_id: '0190-failed-24', event_type: 'failed', occurred_at: plusOffsetMs(createdAt, 61_000), payload: { error: 'boom' } });
  expect(r2.status()).toBe(202);
  const gw = gatewayFor({ teamId: bsA.admin.team_id, operatorId: `op-${bsA.admin.team_id}` });
  const gwApi = await gw.api(worker.baseUrl);
  try {
    ...
    const body = await events.json();
    const types = (body.items as Array<{ event_type: string }>).map((e) => e.event_type);
    expect(types, 'failure path projects [created, running, failed]').toEqual(['created', 'running', 'failed']);
...
test('v0002.25 invalid transitions: terminal-after-terminal, out-of-order tuples, accepted_sequence, cross-team envelope', async () => {
  ...
  await postTaskEvent(exec, worker.baseUrl, task.task_id, { ...base, event_id: '0190-r-25', event_type: 'running', occurred_at: plusOffsetMs(createdAt, 1_000), payload: {} });
  await postTaskEvent(exec, worker.baseUrl, task.task_id, { ...base, event_id: '0190-f-25', event_type: 'finished', occurred_at: plusOffsetMs(createdAt, 61_000), payload: {} });
  await test.step('terminal-after-terminal returns 409 invalid_task_transition', async () => {
    const resp = await postTaskEvent(exec, worker.baseUrl, task.task_id, { ...base, event_id: '0190-f2-25', event_type: 'failed', occurred_at: plusOffsetMs(createdAt, 121_000), payload: {} });
    expect(resp.status(), 'terminal-after-terminal is rejected with 409').toBe(409);
    const body = await resp.json() as { code: string };
    expect(body.code, 'terminal-after-terminal body carries the documented invalid_task_transition code').toBe('invalid_task_transition');
  });
  await test.step('out-of-order tuple with accepted_sequence is rejected with 409; accepted_sequence is not honored', async () => {
    ...
    const body = await resp.json() as { code: string };
    expect(body.code, 'out-of-order body carries the documented invalid_task_transition code').toBe('invalid_task_transition');
  });
  await test.step('cross-team envelope team_id is rejected with 403 team_mismatch', async () => {
    ...
    expect(resp.status(), 'cross-team envelope is rejected with 403 team_mismatch').toBe(403);
    const body = await resp.json() as { code: string };
    expect(body.code, 'cross-team envelope body carries the documented team_mismatch code').toBe('team_mismatch');
  });
});
...
test('v0002.26 deterministic ordering (occurred_at ASC, event_id ASC); retries dedupe; accepted_sequence never overrides', async () => {
  ...
  await postTaskEvent(exec, worker.baseUrl, task.task_id, { ...base, event_id: '0190-tie-0001', event_type: 'running', occurred_at: tieOccurredAt, payload: {} });
  await postTaskEvent(exec, worker.baseUrl, task.task_id, { ...base, event_id: '0190-tie-0002', event_type: 'finished', occurred_at: tieOccurredAt, payload: {} });
  const gw = gatewayFor({ teamId: bsA.admin.team_id, operatorId: `op-${bsA.admin.team_id}` });
  const gwApi = await gw.api(worker.baseUrl);
  try {
    const events = await gwApi.get(`/v1/tasks/${encodeURIComponent(task.task_id)}/events`);
    ...
    const body = await events.json();
    const items = Array.isArray(body.items) ? body.items : [];
    const tieRows = items.filter((e: { occurred_at: string }) => e.occurred_at === tieOccurredAt);
    expect(tieRows.length, 'two events at the same timestamp are present').toBeGreaterThanOrEqual(2);
    const ids = tieRows.map((e: { event_id: string }) => e.event_id);
    expect(ids, 'equal-time rows are ordered by ascending event_id').toEqual([...ids].sort());
  } finally {
    await gwApi.dispose();
  }
});
...
test('v0002.29 assignment ownership survives restart', async () => {
  ...
  const stale = await claimAsExecutor(exec2, worker.baseUrl, task.task_id, 'C-2');
  expect(stale.status(), 'post-restart re-claim by another Executor returns 409').toBe(409);
});
...
test('v0002.31 assigned Executor accepts running/finished; unassigned same-team Executor and cross-team envelope are rejected', async () => {
  ...
  const r1 = await postTaskEvent(exec1, worker.baseUrl, task.task_id, { ...base, event_id: '0190-r-31', event_type: 'running', occurred_at: plusOffsetMs(createdAt, 1_000), payload: {} });
  expect(r1.status(), 'assigned Executor running event accepted with 202').toBe(202);
  const wrongExec = await postTaskEvent(exec2, worker.baseUrl, task.task_id, { ...base, executor_id: exec2.attach()['X-FlowAI-Executor-Id'] ?? '', event_id: '0190-x-31', event_type: 'finished', occurred_at: plusOffsetMs(createdAt, 61_000), payload: {} });
  expect(wrongExec.status(), 'unassigned same-team Executor is rejected with 403 not_assigned').toBe(403);
  const wrongExecBody = await wrongExec.json() as { code: string };
  expect(wrongExecBody.code, 'unassigned same-team body carries the documented not_assigned code').toBe('not_assigned');
  const crossTeam = await postTaskEvent(exec1, worker.baseUrl, task.task_id, { ...base, team_id: 'team-other', event_id: '0190-y-31', event_type: 'finished', occurred_at: plusOffsetMs(createdAt, 121_000), payload: {} });
  expect(crossTeam.status(), 'cross-team envelope is rejected with 403 team_mismatch').toBe(403);
  const crossTeamBody = await crossTeam.json() as { code: string };
  expect(crossTeamBody.code, 'cross-team envelope body carries the documented team_mismatch code').toBe('team_mismatch');
});
...
test('v0002.36 foreign-team task events and self events return non-revealing 404', async () => {
  ...
  await test.step('team-a Executor writing a running event for a team-b task returns non-revealing 404', async () => {
    ...
    expect(resp.status(), 'foreign task event probe is non-revealing 404').toBe(404);
  });
});
...
test('v0002.58 system-owned Executor task-event envelope is verified against the parent task\'s team_id', async () => {
  ...
  const claim = await claimAsExecutor(sysExec, worker.baseUrl, task.task_id, 'C-SYS');
  ...
  await test.step('envelope team_id = parent task\'s team_id is accepted with 202', async () => {
    const resp = await postTaskEvent(sysExec, worker.baseUrl, task.task_id, envelope);
    expect(resp.status()).toBe(202);
  });
  await test.step('envelope team_id = team-other is rejected with 403 team_mismatch', async () => {
    ...
    expect(resp.status(), 'system Executor envelope team_id mismatch is rejected').toBe(403);
    const body = await resp.json() as { code: string };
    expect(body.code, 'cross-team envelope body carries the documented team_mismatch code').toBe('team_mismatch');
  });
});
