...
// helper returns.
async function consumeOpenResponse(response: { status: () => number; text: () => Promise<string> }): Promise<OpenResponse> {
...
// tamperSignature flips the last base64url character of the signature
...
async function captureCanonical404(
  ...
  expect(body, 'canonical 404 body parses to the documented ErrorResponse shape').not.toBeNull();
  if (body) {
    expect(
      body.code,
    ...
    expect(body.request_id.length, 'canonical 404 body request_id is non-empty').toBeGreaterThan(0);
  }
  return resp;
}
...
function assertClosureShapeIdentical(
  ...
  expect(actual.status, `${caseLabel} returns 404 environment_unknown_or_unavailable`).toBe(404);
  ...
  expect(actualBody, `${caseLabel} body parses to ErrorResponse`).not.toBeNull();
  expect(canonicalBody, `canonical body parses to ErrorResponse`).not.toBeNull();
  if (!actualBody || !canonicalBody) {
    return;
  }
  expect(
    actualBody.code,
    `${caseLabel} code is the documented EnvironmentUnknownOrUnavailable code`,
  ).toBe('environment_unknown_or_unavailable');
  expect(
    canonicalBody.code,
    `canonical code is the documented EnvironmentUnknownOrUnavailable code`,
  ).toBe('environment_unknown_or_unavailable');
  expect(
    actualBody.message,
    `${caseLabel} message matches the canonical non-sensitive summary`,
  ).toBe(canonicalBody.message);
  expect(actualBody.message.length, `${caseLabel} message is non-empty`).toBeGreaterThan(0);
  expect(
    typeof actualBody.request_id,
    `${caseLabel} request_id is a non-empty string (no equality asserted)`,
  ).toBe('string');
  expect(actualBody.request_id.length, `${caseLabel} request_id is non-empty`).toBeGreaterThan(0);
  expect(
    typeof canonicalBody.request_id,
    'canonical request_id is a non-empty string',
  ).toBe('string');
  expect(canonicalBody.request_id.length, 'canonical request_id is non-empty').toBeGreaterThan(0);
...
test('v0002.8 environment ownership is independent of execution; claim carries the signed scope token envelope', async () => {
  ...
  try {
    ...
    const envBody = await readJson<{ environment_id: string; team_id: string }>(envSnap);
    expect(envBody.team_id, 'environment carries the verified team_id').toBe(bsA.admin.team_id);
  ...
  const exec = teamExecutorFor({ teamId: bsA.admin.team_id, executorId: uniqueExecutorId() });
  ...
  const claim = await claimAsExecutor(exec, worker.baseUrl, task.task_id, 'C-1');
  expect(claim.status(), 'claim returns 200 with the canonical claim response').toBe(200);
  const body = await readJson<ClaimResponseBody>(claim);
  expect(body.claim, 'claim response field is the literal "claimed"').toBe('claimed');
  expect(body.task.task_id, 'claim task_id matches the requested task').toBe(task.task_id);
  expect(body.task.team_id, 'claim task remains owned by team-a').toBe(bsA.admin.team_id);
  expect(body.task.owner_command_id, 'claim sets owner_command_id to C-1').toBe('C-1');
  expect(body.task.current_state, 'claim projects current_state to created').toBe('created');
  expect(body.task.claimed_at, 'claim sets claimed_at').toBeTruthy();
  expect(body.environment_id, 'claim response carries the assigned environment_id').toBe(envId);
  expect(body.resolved_image, 'claim response carries resolved_image').toBeTruthy();
  expect(body.image_source, 'claim response carries image_source').toBeTruthy();
  expect(body.claimed_at, 'claim response carries a claimed_at timestamp').toBeTruthy();
  const token = body.scope_token;
  expect(token, 'claim response carries a freshly signed scope_token').toBeTruthy();
  if (!token) {
  ...
  const header = JSON.parse(decodeBase64Url(parts[0] ?? '')) as ScopeTokenHeader;
  const claims = JSON.parse(decodeBase64Url(parts[1] ?? '')) as ScopeTokenClaims;
  expect(header.alg, 'protected header alg is inside the allow-listed HMAC set').toMatch(
    /^HS(?:256|384|512)$/,
  );
  expect(header.typ, 'protected header typ is the documented scope-token+json').toBe('scope-token+json');
  expect(header.kid, 'protected header kid is a non-empty opaque key identifier').toBeTruthy();
  expect(claims.team_id, 'token team_id matches the team-a scope').toBe(bsA.admin.team_id);
  expect(claims.environment_id, 'token environment_id matches the assigned environment').toBe(envId);
  expect(claims.task_id, 'token task_id matches the claimed task').toBe(task.task_id);
  expect(claims.audience, 'token audience is the literal environment.open scope').toBe(
    'state-registry.environment.open',
  );
  expect(claims.key_id, 'token payload key_id is a non-empty opaque key identifier').toBeTruthy();
  expect(claims.executor_id, 'token payload executor_id is non-empty').toBeTruthy();
  expect(
    !Number.isNaN(Date.parse(claims.issued_at)) && !Number.isNaN(Date.parse(claims.expiry)),
...
test('v0002.9 team-owned environment definition is stored with immutable team_id and exposes non-secret values only', async () => {
  ...
  try {
    ...
    expect(body.team_id, 'environment carries the verified team_id').toBe(bsA.admin.team_id);
    expect(body.values, 'non-secret values are present in the response').toEqual({
      REGION: 'us-east-1',
      TIER: 'gold',
    });
    expect(body.revision, 'environment revision is a positive integer').toBeGreaterThanOrEqual(1);
    expect(Array.isArray(body.secrets), 'environment secrets array is present').toBe(true);
    envId = body.environment_id;
  } finally {
    await gwApi.dispose();
  }
  const gwList = gatewayFor({ teamId: bsA.admin.team_id, operatorId: `op-${bsA.admin.team_id}` });
  ...
  try {
    ...
    const body = await readJson<{ items: Array<{ environment_id: string; team_id: string }> }>(snap);
    ...
    if (env) {
      expect(env.team_id, 'listed environment stays owned by team-a').toBe(bsA.admin.team_id);
    }
  } finally {
...
test('v0002.10 logical secret create + replace returns opaque key_id / key_version metadata without plaintext', async () => {
  ...
  try {
    ...
    const create = await gwApi.post(`/v1/environments/${encodeURIComponent(envId)}/secrets`, {
      data: {
        name: 'OPENAI_API_KEY',
    ...
    expect(createBody.secret.secret_id, 'create returns secret_id').toBeTruthy();
    secretId = createBody.secret.secret_id;
    expect(createBody.version.version, 'create returns version number >= 1').toBeGreaterThanOrEqual(1);
    expect(createBody.version.key_id, 'create returns opaque key_id').toBeTruthy();
    expect(createBody.version.key_version, 'create returns opaque key_version >= 1').toBeGreaterThanOrEqual(1);
    expect(createBody.version.created_at, 'create returns created_at timestamp').toBeTruthy();
    ...
    const replaceBody = await readJson<{
      version: { version: number; key_id: string; key_version: number; created_at: string };
    }>(replaceSnap);
    expect(
      replaceBody.version.version > createBody.version.version,
      'replace returns a strictly higher version number (monotonic)',
    ).toBe(true);
    expect(replaceBody.version.key_id, 'replace returns opaque key_id').toBeTruthy();
    expect(replaceBody.version.key_version, 'replace returns opaque key_version >= 1').toBeGreaterThanOrEqual(1);
...
test('v0002.11 assigned Executor opens environment via scope token; foreign Executor returns non-revealing 404', async () => {
  ...
  const claim = await claimAsExecutor(execAssigned, worker.baseUrl, task.task_id, 'C-1');
  expect(claim.status(), 'claim returns 200 with the canonical claim response').toBe(200);
  const claimBody = await readJson<ClaimResponseBody>(claim);
  const scopeToken = claimBody.scope_token;
  expect(scopeToken, 'claim response carries the freshly signed scope_token').toBeTruthy();
  if (!scopeToken) {
    throw new Error('v0002.11: claim response missing scope_token');
  }
  const claims = decodeClaims(scopeToken);
  expect(claims.executor_id, 'token payload executor_id matches the assigned Executor identity').toBe(
    execAssigned.attach()['X-FlowAI-Executor-Id'] ?? '',
  );
  ...
  expect(opened.status, 'assigned Executor open with the claim-issued token returns 200').toBe(200);
  const openedBody = opened.json as OpenEnvironmentBody | null;
  expect(openedBody, 'open response body parses to OpenEnvironmentResponse').not.toBeNull();
  if (openedBody) {
    expect(openedBody.team_id, 'open response team_id matches the same team').toBe(bsA.admin.team_id);
    expect(openedBody.environment_id, 'open response environment_id matches the assigned env').toBe(envId);
    expect(openedBody.task_id, 'open response task_id matches the claimed task').toBe(task.task_id);
    expect(openedBody.executor_id, 'open response executor_id matches the assigned Executor').toBe(
      execAssigned.attach()['X-FlowAI-Executor-Id'] ?? '',
    );
  }
  ...
  assertClosureShapeIdentical(foreignOpen, canonical, 'foreign same-team Executor with assigned token');
...
test('v0002.38 environment reads and writes are team-scoped; foreign probes return non-revealing 404 with zero encrypt ops', async () => {
  ...
  const gwBOwner = gatewayFor({ teamId: bsB.admin.team_id, operatorId: `op-${bsB.admin.team_id}` });
  const gwBOwnerApi = await gwBOwner.api(worker.baseUrl);
  try {
    const read = await gwBOwnerApi.get(`/v1/environments/${encodeURIComponent(envIdB)}`);
    expect(read.status(), 'team-b reads its own environment with 200').toBe(200);
    const readSnap = await snapshotResponse(read);
    const readBody = await readJson<{ team_id: string; environment_id: string }>(readSnap);
    expect(readBody.team_id, 'team-b read returns team-b team_id').toBe(bsB.admin.team_id);
    expect(readBody.environment_id, 'team-b read returns the requested environment_id').toBe(envIdB);
  } finally {
  ...
  const gwA = gatewayFor({ teamId: bsA.admin.team_id, operatorId: `op-${bsA.admin.team_id}` });
  const gwCanonicalApi = await gwA.api(worker.baseUrl);
  let canonical: OpenResponse;
  try {
    const resp = await gwCanonicalApi.get(`/v1/environments/${encodeURIComponent(UnknownEnvId)}`);
    canonical = await consumeOpenResponse(resp);
    expect(canonical.status, 'canonical environment read returns 404').toBe(404);
    const canonicalBody = canonical.json as OpenErrorBody | null;
    expect(
      canonicalBody?.code,
  ...
  try {
    const resp = await gwAApi.get(`/v1/environments/${encodeURIComponent(envIdB)}`);
    const raw = await consumeOpenResponse(resp);
    assertClosureShapeIdentical(raw, canonical, 'team-a probe of team-b environment');
  } finally {
    await gwAApi.dispose();
  }
  const after = await snapshotDecryptOps(worker.baseUrl);
  expect(after.raw, 'team-a probe of team-b environment records zero encrypt operations').toBe(
    baseline.raw,
  );
});
...
test('v0002.39 logical secrets and version metadata reads are team-scoped; foreign probes return non-revealing 404 with zero encrypt ops', async () => {
  ...
  try {
    ...
    const envBody = await readJson<{ environment_id: string }>(envSnap);
    envIdB = envBody.environment_id;
    const secret = await gwBApi.post(`/v1/environments/${encodeURIComponent(envIdB)}/secrets`, {
      data: {
        name: 'OPENAI_API_KEY',
        value: 'team-b-secret',
        scope: { project_id: null, task_id: null, parent_task_id: null },
    ...
    const secretBody = await readJson<{ secret: { secret_id: string } }>(secretSnap);
  ...
  const gwBOwnerApi = await gwBOwner.api(worker.baseUrl);
  try {
    const list = await gwBOwnerApi.get(`/v1/environments/${encodeURIComponent(envIdB)}/secrets`);
    ...
    expect(versions.status(), 'team-b lists its own secret versions with 200').toBe(200);
  ...
  let canonical: OpenResponse;
  try {
    const resp = await gwCanonicalApi.get(`/v1/environments/${encodeURIComponent(UnknownEnvId)}/secrets`);
    canonical = await consumeOpenResponse(resp);
  ...
  const gwAApi = await gwA.api(worker.baseUrl);
  try {
    const resp = await gwAApi.get(`/v1/environments/${encodeURIComponent(envIdB)}/secrets`);
    const raw = await consumeOpenResponse(resp);
    assertClosureShapeIdentical(raw, canonical, 'team-a probe of team-b secret metadata');
  } finally {
    await gwAApi.dispose();
  }
  const after = await snapshotDecryptOps(worker.baseUrl);
  expect(after.raw, 'team-a probe of team-b secret metadata records zero encrypt operations').toBe(
    baseline.raw,
  );
});
...
test('v0002.40 open-environment boundary verifies every team-bound claim before decrypt; invalid tokens return the same 404 shape', async () => {
  ...
  expect(positive.status, 'positive control: assigned Executor open returns 200').toBe(200);
  ...
  const canonical = await captureCanonical404(exec, worker.baseUrl, envId, task.task_id);
  ...
  const mismatchTokens: Case[] = [
    { name: 'tampered-signature', token: tamperSignature(scopeToken) },
    {
      ...
      token: rewriteClaims(scopeToken, (c) => ({ ...c, task_id: `${c.task_id}-mismatch` })),
    },
    {
      name: 'environment-id-claim-mismatch',
      token: rewriteClaims(scopeToken, (c) => ({ ...c, environment_id: 'env-foreign-shape' })),
    },
    {
      name: 'wrong-audience',
    ...
    },
    {
      ...
      token: rewriteHeader(scopeToken, (h) => ({ alg: h.alg, kid: `${h.kid}-mismatch`, typ: h.typ })),
    },
    {
      name: 'alg-not-allow-listed',
      token: rewriteHeader(scopeToken, (h) => ({ alg: 'none', kid: h.kid, typ: h.typ })),
    },
    {
      name: 'typ-not-scope-token',
      token: rewriteHeader(scopeToken, (h) => ({ alg: h.alg, kid: h.kid, typ: 'JWT' })),
    },
  ];
  ...
  for (const c of mismatchTokens) {
    ...
    const after = await snapshotDecryptOps(worker.baseUrl);
    expect(after.raw, `${c.name} records zero decrypt operations`).toBe(before.raw);
  }
  ...
  const emptyToken = await openEnvironmentAsExecutor(exec, worker.baseUrl, envId, task.task_id, null);
  assertClosureShapeIdentical(emptyToken, canonical, 'missing-token');
  const noTokenAfter = await snapshotDecryptOps(worker.baseUrl);
  expect(noTokenAfter.raw, 'missing-token records zero decrypt operations').toBe(negativeBefore.raw);
});
...
test('v0002.45 scope-token security: every invalid case returns the documented 404 shape with zero decrypt ops', async () => {
  ...
  const exec1 = teamExecutorFor({ teamId: bsA.admin.team_id, executorId: uniqueExecutorId() });
  ...
  const claim = await claimAsExecutor(exec1, worker.baseUrl, task.task_id, 'C-1');
  ...
  const canonical = await captureCanonical404(exec1, worker.baseUrl, envId, task.task_id);
  ...
  const cases: Case[] = [
    ...
    { name: 'tampered-mac', token: tamperSignature(scopeToken) },
    {
      ...
      token: rewriteHeader(scopeToken, (h) => ({ alg: h.alg, kid: `${h.kid}-tampered`, typ: h.typ })),
    },
    {
      name: 'wrong-audience',
      token: rewriteClaims(scopeToken, (c) => ({
    ...
    },
    {
      ...
      token: rewriteClaims(scopeToken, (c) => ({ ...c, team_id: 'team-foreign-shape' })),
    },
    {
    ...
    },
    {
      ...
      token: rewriteClaims(scopeToken, (c) => {
        const future = new Date(Date.parse(c.issued_at) + 6 * 60 * 1000).toISOString();
  ...
  const exec2 = teamExecutorFor({ teamId: bsA.admin.team_id, executorId: uniqueExecutorId() });
  ...
  const cross = await openEnvironmentAsExecutor(exec2, worker.baseUrl, envId, task.task_id, scopeToken);
  assertClosureShapeIdentical(cross, canonical, 'server-issued token under different Executor identity');
...
test('v0002.46 operator stores a task-owned environment definition with parent_task_id', async () => {
  ...
  try {
    const resp = await gwApi.post('/v1/environments', {
      data: {
        name: 'env-v0002-46',
        scope: { project_id: null, task_id: null, parent_task_id: task.task_id },
        values: { REGION: 'us-east-1' },
      },
    });
    expect(resp.status(), 'POST /v1/environments returns 201').toBe(201);
    ...
    const body = await readJson<{
      environment_id: string;
      team_id: string;
      scope: { project_id: string | null; task_id: string | null; parent_task_id: string | null };
    }>(snap);
    expect(body.team_id, 'task-owned environment carries the verified team_id').toBe(bsA.admin.team_id);
    expect(
  ...
  const gwRead = gatewayFor({ teamId: bsA.admin.team_id, operatorId: `op-${bsA.admin.team_id}` });
  ...
  try {
    ...
    const taskOwned = items.find((it) => it.scope.parent_task_id === task.task_id);
    expect(taskOwned, 'team-a listing returns the task-owned environment').toBeDefined();
    if (taskOwned) {
      expect(taskOwned.team_id, 'task-owned listing row stays owned by team-a').toBe(bsA.admin.team_id);
    }
  } finally {
...
test('v0002.47 operator stores a task-owned secret with REQUIRED parent task applicability', async () => {
  ...
  try {
    ...
    const envBody = await readJson<{ environment_id: string }>(envSnap);
    envId = envBody.environment_id;
    const secret = await gwApi.post(`/v1/environments/${encodeURIComponent(envId)}/secrets`, {
      data: {
        name: 'OPENAI_API_KEY',
    ...
    expect(secretBody.secret.environment_id, 'secret belongs to the task-owned environment').toBe(envId);
    expect(secretBody.secret.team_id, 'secret carries the verified team_id').toBe(bsA.admin.team_id);
    expect(
      secretBody.secret.scope.parent_task_id,
      'secret scope carries the parent task_id',
    ).toBe(task.task_id);
    expect(secretBody.version.version, 'secret version is >= 1').toBeGreaterThanOrEqual(1);
    expect(secretBody.version.key_id, 'secret version carries opaque key_id').toBeTruthy();
    expect(secretBody.version.key_version, 'secret version carries opaque key_version').toBeGreaterThanOrEqual(1);
  ...
  try {
    ...
    expect(versions.status(), 'team-scoped secret version listing returns 200').toBe(200);
    const versionsSnap = await snapshotResponse(versions);
    ...
    expect(items.length, 'task-owned secret version listing contains at least one immutable version').toBeGreaterThanOrEqual(1);
    const versionsText = await versionsSnap.text();
    expect(
...
test('v0002.48 task-owned environment rejects a foreign-team parent_task_id with the same non-revealing 404 shape', async () => {
  ...
  try {
    const resp = await gwCanonicalApi.post('/v1/environments', {
      data: {
        ...
        scope: { project_id: null, task_id: null, parent_task_id: UnknownTaskId },
        values: { REGION: 'us-east-1' },
      },
    });
    canonical = await consumeOpenResponse(resp);
    expect(canonical.status, 'canonical environment create returns 404').toBe(404);
    const canonicalBody = canonical.json as OpenErrorBody | null;
    expect(canonicalBody, 'canonical body parses to ErrorResponse').not.toBeNull();
    if (canonicalBody) {
      expect(canonicalBody.code, 'canonical body carries environment_unknown_or_unavailable code').toBe(
        'environment_unknown_or_unavailable',
      );
      expect(typeof canonicalBody.request_id, 'canonical body request_id is a string').toBe('string');
      expect(canonicalBody.request_id.length, 'canonical body request_id is non-empty').toBeGreaterThan(0);
    }
  } finally {
    await gwCanonicalApi.dispose();
  }
  ...
  try {
    const resp = await gwApi.post('/v1/environments', {
      data: {
        ...
        scope: { project_id: null, task_id: null, parent_task_id: teamBTask.task_id },
        values: { REGION: 'us-east-1' },
      },
    });
    const raw = await consumeOpenResponse(resp);
  ...
  const after = await snapshotDecryptOps(worker.baseUrl);
  expect(after.raw, 'foreign parent_task_id records zero encrypt operations').toBe(baseline.raw);
});
...
test('v0002.49 task-owned environment can be opened ONLY by the parent task\'s assigned Executor; siblings return 404', async () => {
  ...
  try {
    const env = await gwApi.post('/v1/environments', {
      data: {
        name: 'env-v0002-49',
        scope: { project_id: null, task_id: null, parent_task_id: task1.task_id },
        values: { REGION: 'us-east-1' },
      },
    });
    expect(env.status(), 'POST /v1/environments returns 201').toBe(201);
  ...
  const execParent = teamExecutorFor({ teamId: bsA.admin.team_id, executorId: uniqueExecutorId() });
  ...
  const execSibling = teamExecutorFor({ teamId: bsA.admin.team_id, executorId: uniqueExecutorId() });
  ...
  // ingestPendingTask does not link the task to the environment, so we
  // do a separate task ingestion that references envId for the parent
  // task only. The sibling task is intentionally NOT linked to the
  // task-owned environment.
  const taskParent = await ingestPendingTask(
    listenerFor({
      teamId: bsA.admin.team_id,
      listenerIdentity: bsA.listenerIdentity,
      sourceSystemId: bsA.sourceSystem.source_system_id,
  ...
  const parentClaim = await claimAsExecutor(execParent, worker.baseUrl, taskParent.task_id, 'C-parent');
  ...
  expect(parentToken, 'parent task claim carries a freshly signed scope_token').toBeTruthy();
  if (!parentToken) {
    throw new Error('v0002.49: parent claim response missing scope_token');
  }

  const siblingClaim = await claimAsExecutor(execSibling, worker.baseUrl, task2.task_id, 'C-sibling');
  expect(siblingClaim.status(), 'sibling task claim returns 200').toBe(200);
  const siblingClaimBody = await readJson<ClaimResponseBody>(siblingClaim);
  const siblingToken = siblingClaimBody.scope_token;
  expect(siblingToken, 'sibling task claim carries a freshly signed scope_token').toBeTruthy();
  if (!siblingToken) {
    throw new Error('v0002.49: sibling claim response missing scope_token');
  }
  ...
  expect(parentBody, 'parent open body parses to OpenEnvironmentResponse').not.toBeNull();
  if (parentBody) {
    expect(parentBody.task_id, 'parent open carries the parent task_id').toBe(taskParent.task_id);
    expect(parentBody.environment_id, 'parent open carries the task-owned environment_id').toBe(envId);
  }

  // Negative control: sibling task's assigned Executor opens with its
  // own freshly signed VALID token (NOT a malformed token) under the
  // task-owned environment_id. The Registry must reject the request
  // and return the canonical 404 closure shape with zero decrypt
  // operations. The canonical is captured under the sibling Executor
  // identity so the closure shape comparison is on identical transport.
  const canonical = await captureCanonical404(execSibling, worker.baseUrl, envId, task2.task_id);
  ...
  assertClosureShapeIdentical(siblingOpen, canonical, 'sibling task with valid token');
  const siblingAfter = await snapshotDecryptOps(worker.baseUrl);
  expect(siblingAfter.raw, 'sibling task open records zero decrypt operations').toBe(siblingBefore.raw);
});
...
test('v0002.77 AES-256-GCM startup fails closed: configured key is required before any secret write or open', async () => {
  ...
  expect(captured, 'startRegistryWorker fails closed when STATE_REGISTRY_AES_KEY_HEX is empty').not.toBeNull();
  const capturedDiagnostics =
    captured && 'diagnostics' in captured && typeof captured.diagnostics === 'string'
      ? captured.diagnostics
      : '';
  const errorText = captured ? `${captured.message}\n${capturedDiagnostics}` : '';
  expect(errorText, 'fail-closed startup error carries the missing-key sentinel').toMatch(
