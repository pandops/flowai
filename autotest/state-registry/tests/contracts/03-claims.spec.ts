...
async function claimAs(
  exec: { api: (base: string) => Promise<import('@playwright/test').APIRequestContext>; attach: () => Readonly<Record<string, string>> },
  baseUrl: string,
  taskId: string,
  commandId: string,
) {
...
test('v0002.4 concurrent claim is atomic: exactly one winner returns 200 and the other returns 409; exactly one created event exists', async () => {
  ...
  const execA = teamExecutorFor({ teamId: bsA.admin.team_id, executorId: uniqueExecutorId() });
  const execB = teamExecutorFor({ teamId: bsA.admin.team_id, executorId: uniqueExecutorId() });
  ...
  await test.step('two concurrent claims produce exactly the sorted multiset [200, 409]', async () => {
    ...
    expect(statuses, 'concurrent claim multiset is exactly [200, 409]').toEqual([200, 409]);
    const winnerIndex = [a, b].findIndex((r) => r.status() === 200);
    const winner = winnerIndex === 0 ? a : b;
    const winnerBody = await winner.json() as { task: { executor_id: string | null } };
    const claimedExecutorId = winnerBody.task.executor_id;
    expect(typeof claimedExecutorId, 'winning claim body carries the claiming executor_id').toBe('string');
  ...
  await test.step('canonical task has exactly one Registry-appended created event with the winner executor_id', async () => {
    expect(winnerExecutorId, 'winner executor_id is captured from the winning claim').toBeTruthy();
    const gw = gatewayFor({
      teamId: bsA.admin.team_id,
    ...
    try {
      const events = await gwApi.get(`/v1/tasks/${encodeURIComponent(task.task_id)}/events`);
      ...
      expect(created.length, 'exactly one Registry-appended created event').toBe(1);
      const createdEvent = created[0];
      if (!createdEvent) {
        throw new Error('created event array is unexpectedly empty after length assertion');
      }
      expect(createdEvent.executor_id, 'created event has non-null executor_id').toBeTruthy();
      expect(createdEvent.executor_id, 'created event executor_id matches the winning claim').toBe(winnerExecutorId);
    } finally {
      await gwApi.dispose();
    }
  });
...
test('v0002.21 successful claim atomically removes the task from the winner\'s scope of discovery', async () => {
  ...
  await test.step('pre-claim discovery: the pending task is visible to the team-a Executor', async () => {
    ...
    try {
      const resp = await api.get(`/v1/executors/${encodeURIComponent(execId)}/tasks?tag=openhands`);
      expect(resp.status(), 'pre-claim discovery returns 200').toBe(200);
      const body = await resp.json();
      const items = Array.isArray((body as { items: unknown }).items) ? (body as { items: Array<{ task_id: string }> }).items : [];
      const ids = items.map((it) => it.task_id);
      expect(ids, 'pre-claim discovery returns the pending task').toContain(task.task_id);
    } finally {
      await api.dispose();
    }
  });
  ...
  await test.step('POST /v1/executors/{id}/claim returns 200 with the canonical claim body', async () => {
    ...
    expect(body.task.task_id, 'task.task_id is the requested task').toBe(task.task_id);
    expect(body.task.team_id, 'task stays owned by team-a').toBe(bsA.admin.team_id);
    expect(body.task.owner_command_id, 'tasks.owner_command_id is set to C-1').toBe('C-1');
    expect(body.task.executor_id, 'tasks.executor_id is set on claim').toBeTruthy();
    expect(body.task.claimed_at, 'tasks.claimed_at is set on claim').toBeTruthy();
    ...
    expect(body.task.current_state, 'projection becomes created').toBe('created');
  });
  ...
  await test.step('post-claim discovery: the task is removed from the winner\'s scope of discovery', async () => {
    ...
    try {
      const resp = await api.get(`/v1/executors/${encodeURIComponent(execId)}/tasks?tag=openhands`);
      expect([200, 204], 'post-claim discovery returns 200 (empty) or 204').toContain(resp.status());
      if (resp.status() === 200) {
        const body = await resp.json() as { items: Array<{ task_id: string }> };
        const ids = body.items.map((it) => it.task_id);
        expect(ids, 'post-claim discovery does not contain the claimed task').not.toContain(task.task_id);
      }
    } finally {
      await api.dispose();
    }
  ...
  await test.step('stale claim by the same Executor with a different command_id returns exact 409 task_already_claimed', async () => {
    ...
    expect(stale.status(), 'stale claim returns EXACT 409 task_already_claimed').toBe(409);
    const staleBody = await stale.json() as { code: string };
    expect(staleBody.code, 'stale claim body carries the documented task_already_claimed code').toBe('task_already_claimed');
  });
  ...
  await test.step('Gateway read confirms the created event is appended atomically (no dispatched event)', async () => {
    ...
    const gwApi = await gw.api(worker.baseUrl);
    try {
      const taskView = await gwApi.get(`/v1/tasks/${encodeURIComponent(task.task_id)}`);
      expect(taskView.status(), 'trusted Gateway reads the canonical task with 200').toBe(200);
      const viewBody = await taskView.json() as {
        current_state: string;
        owner_command_id: string | null;
        executor_id: string | null;
      };
      expect(viewBody.current_state, 'task is projected current_state = created').toBe('created');
      ...
      expect(types, 'no dispatched event exists in the ordered history').not.toContain('dispatched');
    } finally {
      await gwApi.dispose();
    }
  });
...
test('v0002.22 stale claim from a different Executor returns 409 task_already_claimed with no mutation', async () => {
  ...
  await test.step('first claim wins with 200; second same-team claim returns EXACT 409 task_already_claimed', async () => {
    ...
    const staleBody = await stale.json() as { code: string };
    expect(staleBody.code, 'stale claim body carries EXACT task_already_claimed code').toBe('task_already_claimed');
  });
});
...
test('v0002.30 the FIRST created event is Registry-appended atomically on successful claim', async () => {
  ...
  await test.step('claim returns 200 with the FIRST created event appended in the same transaction', async () => {
    ...
    const body = await claim.json() as {
      task: { current_state: string; owner_command_id: string | null; executor_id: string | null; claimed_at: string | null };
    };
    expect(body.task.current_state, 'projection becomes created').toBe('created');
    expect(body.task.owner_command_id).toBe('C-30');
    expect(body.task.claimed_at, 'claimed_at is set in the same transaction').toBeTruthy();
    expect(body.task.executor_id, 'task.executor_id is set in the claim transaction').toBeTruthy();
  ...
  await test.step('trusted Gateway read confirms exactly one Registry-appended created event with the claiming executor_id', async () => {
    ...
    try {
      ...
      expect(createdEvent.executor_id, 'created event has executor_id equal to the claiming Executor').toBe(claimedExecutorId);
      const types = items.map((e) => e.event_type);
      expect(types, 'no dispatched event exists in the ordered history').not.toContain('dispatched');
    } finally {
      await gwApi.dispose();
...
test('v0002.35 foreign team task identifier returns non-revealing 404 (same shape as unknown)', async () => {
  ...
  await test.step('team-a Executor claim for team-b task returns non-revealing 404; unknown task returns identical 404 shape', async () => {
    const foreign = await claimAs(exec, worker.baseUrl, foreignTask.task_id, 'C-X');
    const unknown = await claimAs(exec, worker.baseUrl, '00000000-0000-0000-0000-000000000000', 'C-Y');
    expect(foreign.status(), 'foreign task claim is non-revealing 404').toBe(404);
    expect(unknown.status(), 'unknown task claim returns the same 404 shape').toBe(404);
    const foreignBody = await foreign.text();
    const unknownBody = await unknown.text();
    expect(foreignBody, 'foreign and unknown 404 responses have identical bodies').toBe(unknownBody);
  });
});
...
test('v0002.57 system-owned Executor claims a team-a task; team-a competitor receives 409', async () => {
  ...
  const teamExec = teamExecutorFor({ teamId: bsA.admin.team_id, executorId: uniqueExecutorId() });
  ...
  await test.step('system Executor wins the claim; team-a competitor receives EXACT 409 task_already_claimed', async () => {
    const sysWin = await claimAs(sysExec, worker.baseUrl, task.task_id, 'C-SYS');
    expect(sysWin.status()).toBe(200);
    const teamLose = await claimAs(teamExec, worker.baseUrl, task.task_id, 'C-TEAM');
    ...
    expect(teamLoseBody.code, 'team-a competitor body carries EXACT task_already_claimed code').toBe('task_already_claimed');
  });
});
...
test('v0002.68 claim succeeds only when the requested task is the OLDEST eligible pending task', async () => {
  ...
  await test.step('claim for T-newer with command_id=C-n returns EXACT 409 older_task_must_be_claimed_first', async () => {
    ...
    expect(resp.status(), 'older_task_must_be_claimed_first is EXACT 409').toBe(409);
    const respBody = await resp.json() as { code: string };
    expect(respBody.code, 'older_task_must_be_claimed_first body carries EXACT code').toBe('older_task_must_be_claimed_first');
  });
  await test.step('claim for T-older succeeds with 200', async () => {
    ...
    expect(olderWin.status(), 'oldest eligible task claim returns 200').toBe(200);
  });
});
...
test('v0002.69 same (task_id, command_id) retry returns the original 200 claimed body with no new event', async () => {
  ...
  await test.step('first claim returns 200; identical retry returns 200 with the same owner_command_id', async () => {
    ...
    const firstBody = await first.json() as {
      task: { owner_command_id: string | null; executor_id: string | null; resolved_image: unknown; image_source: string | null };
    };
    const retry = await claimAs(exec, worker.baseUrl, task.task_id, 'C-stable');
    expect(retry.status(), 'same (task_id, command_id) retry is idempotent and returns 200').toBe(200);
    const retryBody = await retry.json() as {
      task: { owner_command_id: string | null; executor_id: string | null; resolved_image: unknown; image_source: string | null };
    };
    expect(retryBody.task.owner_command_id, 'owner_command_id is preserved on retry').toBe(firstBody.task.owner_command_id);
    expect(retryBody.task.executor_id, 'executor_id is preserved on retry').toBe(firstBody.task.executor_id);
    expect(retryBody.task.resolved_image, 'resolved_image is preserved on retry').toBe(firstBody.task.resolved_image);
    expect(retryBody.task.image_source, 'image_source is preserved on retry').toBe(firstBody.task.image_source);
  });
  await test.step('trusted Gateway read confirms the retry did NOT append a new event', async () => {
    ...
    try {
      ...
      const items = readEvents(eventsBody);
      const created = items.filter((e) => e.event_type === 'created');
      expect(created.length, 'identical retry does not append a new created event').toBe(1);
    } finally {
      await gwApi.dispose();
    }
  });
  await test.step('same task_id with a DIFFERENT command_id returns EXACT 409 task_already_claimed', async () => {
    ...
    expect(other.status(), 'same task_id with a different command_id returns EXACT 409').toBe(409);
    const otherBody = await other.json() as { code: string };
    expect(otherBody.code, 'different command_id body carries EXACT task_already_claimed code').toBe('task_already_claimed');
  });
});
...
test('v0002.70 one command_id may own multiple tasks; each task is limited to one command', async () => {
  ...
  await test.step('two claims under the SAME command_id both return 200 with separate created events', async () => {
    const c1 = await claimAs(exec, worker.baseUrl, t1.task_id, 'C-multi');
    expect(c1.status()).toBe(200);
    const c2 = await claimAs(exec, worker.baseUrl, t2.task_id, 'C-multi');
    expect(c2.status()).toBe(200);
    const b2 = await c2.json() as { task: { owner_command_id: string | null } };
    expect(b2.task.owner_command_id, 'second task also carries the SAME command_id').toBe('C-multi');
  });
});
...
test('v0002.71 supersedes the prior atomic-claim definition with a tighter precondition re-verifies concurrent claim atomicity', async () => {
  ...
  await test.step('two concurrent claims produce exactly the sorted multiset [200, 409]', async () => {
    ...
    const statuses = [a.status(), b.status()].sort((x, y) => x - y);
    expect(statuses, 'v0002.71 multiset is exactly [200, 409]').toEqual([200, 409]);
    const winnerIndex = [a, b].findIndex((r) => r.status() === 200);
    const winner = winnerIndex === 0 ? a : b;
    const winnerBody = await winner.json() as { task: { executor_id: string | null } };
    expect(winnerBody.task.executor_id, 'winning claim body carries the claiming executor_id').toBeTruthy();
  ...
  await test.step('trusted Gateway read confirms exactly one Registry-appended created event with the winner executor_id', async () => {
    ...
    try {
      ...
      expect(createdEvent.executor_id, 'created event has executor_id equal to the winning claim').toBe(winnerExecutorId);
    } finally {
      await gwApi.dispose();
    }
  });
...
test('v0002.72 FIFO tie-break by task_id when two pending tasks share the same ingested_at', async () => {
  ...
  try {
    const taView = await gwApi.get(`/v1/tasks/${encodeURIComponent(ta.task_id)}`);
    expect(taView.status(), 'trusted Gateway reads T-a canonical task with 200').toBe(200);
    const taBody = await taView.json() as { task_id: string; ingested_at: string };
    observedTupleA = { task_id: taBody.task_id, ingested_at: taBody.ingested_at };

    const tbView = await gwApi.get(`/v1/tasks/${encodeURIComponent(tb.task_id)}`);
    expect(tbView.status(), 'trusted Gateway reads T-b canonical task with 200').toBe(200);
    const tbBody = await tbView.json() as { task_id: string; ingested_at: string };
    observedTupleB = { task_id: tbBody.task_id, ingested_at: tbBody.ingested_at };
  } finally {
    await gwApi.dispose();
  }
  ...
  await test.step('honest RED: equal-time tie fixture is the precondition for the task_id tie-break assertion', async () => {
    ...
    if (observedTupleA.ingested_at !== observedTupleB.ingested_at) {
      throw new Error(
        ...
        observedTupleA.ingested_at + ' and tb=' + observedTupleB.ingested_at + '). ' +
  ...
  await test.step('discovery yields the supported FIFO order (ingested_at ASC, task_id ASC)', async () => {
    const expectedOrder = [observedTupleA, observedTupleB]
      .sort((a, b) => {
      ...
      .map((x) => x.task_id);
    const expectedById = new Map(expectedOrder.map((id, idx) => [id, idx] as const));

    const api = await exec.api(worker.baseUrl);
    try {
      const resp = await api.get(`/v1/executors/${encodeURIComponent(exec.attach()['X-FlowAI-Executor-Id'] ?? '')}/tasks?tag=openhands`);
      expect(resp.status(), 'discovery returns 200 for the registered Executor').toBe(200);
      const body = await resp.json();
      const items = Array.isArray((body as { items: unknown }).items)
        ? (body as { items: Array<{ task_id: string; team_id: string; required_tag: string; current_state: string; ingested_at: string }> }).items
        : [];
      const ids = items.map((it) => it.task_id);
      const idxA = ids.indexOf(observedTupleA.task_id);
      const idxB = ids.indexOf(observedTupleB.task_id);
      expect(idxA, 'T-a appears in the discovery page').toBeGreaterThanOrEqual(0);
      expect(idxB, 'T-b appears in the discovery page').toBeGreaterThanOrEqual(0);
      expect(expectedById.get(observedTupleA.task_id), 'T-a is the first expected FIFO entry').toBe(0);
      expect(expectedById.get(observedTupleB.task_id), 'T-b is the second expected FIFO entry').toBe(1);
      expect(idxA, 'discovery order matches the FIFO tuple ordering (idxA < idxB)').toBeLessThan(idxB);
    } finally {
      await api.dispose();
    }
  });

  await test.step('first claim for T-b returns EXACT 409 older_task_must_be_claimed_first with no mutation', async () => {
    const resp = await claimAs(exec, worker.baseUrl, observedTupleB.task_id, 'C-72-b-first');
    expect(resp.status(), 'older_task_must_be_claimed_first is EXACT 409').toBe(409);
    const respBody = await resp.json() as { code: string };
    expect(respBody.code, 'older_task_must_be_claimed_first body carries EXACT code').toBe('older_task_must_be_claimed_first');
    const gwApi2 = await gw.api(worker.baseUrl);
    try {
      const tbView = await gwApi2.get(`/v1/tasks/${encodeURIComponent(observedTupleB.task_id)}`);
      expect(tbView.status(), 'T-b canonical task read still returns 200').toBe(200);
      const tbRead = await tbView.json() as { owner_command_id: string | null; executor_id: string | null; current_state: string };
      expect(tbRead.owner_command_id, 'T-b owner_command_id remains NULL after the rejected claim').toBeNull();
      expect(tbRead.executor_id, 'T-b executor_id remains NULL after the rejected claim').toBeNull();
      expect(tbRead.current_state, 'T-b current_state remains pending after the rejected claim').toBe('pending');
    } finally {
      await gwApi2.dispose();
    }
  });

  await test.step('claim for T-a returns 200 with the FIRST Registry-appended created event', async () => {
    const resp = await claimAs(exec, worker.baseUrl, observedTupleA.task_id, 'C-72-a');
    expect(resp.status(), 'oldest eligible task claim returns 200').toBe(200);
  });

  await test.step('claim for T-b returns 200 after T-a is claimed (T-b is now the oldest eligible pending task)', async () => {
    const resp = await claimAs(exec, worker.baseUrl, observedTupleB.task_id, 'C-72-b');
    expect(resp.status(), 'remaining eligible task claim returns 200').toBe(200);
  });
});
...
test('v0002.73 claim persists resolved_image and image_source at claim time', async () => {
  ...
  await test.step('claim response carries resolved_image and image_source from team-default precedence', async () => {
    ...
    const body = await resp.json() as {
      resolved_image: unknown;
      image_source: string | null;
      task: { resolved_image: unknown; image_source: string | null; current_state: string };
    };
    expect(body.resolved_image, 'resolved_image is present in claim response').toBeTruthy();
    expect(body.image_source, 'image_source is present in claim response').toBe('team_default');
    expect(body.task.resolved_image, 'tasks.resolved_image is persisted at claim time').toBe(body.resolved_image);
    expect(body.task.image_source, 'tasks.image_source is persisted at claim time').toBe('team_default');
    expect(body.task.current_state, 'task is projected current_state = created').toBe('created');
  });
});
