// Container runtime helpers for the state-registry autotest suite.
...
export interface ContainerRuntime {
  ...
  inspectMappedHostPort(handle: ContainerHandle, containerPort: number): Promise<number>;
...
async function runShell(bin: string, args: string[]): Promise<string> {
  return await new Promise<string>((resolve, reject) => {
    ...
    child.once('close', (code: number | null) => {
      ...
      reject(new ContainerRuntimeError(`${bin} ${args.join(' ')}`, code ?? -1, out, err));
    });
  });
}
...
class DockerRuntime implements ContainerRuntime {
  ...
  async inspectMappedHostPort(handle: ContainerHandle, containerPort: number): Promise<number> {
    ...
    if (!Number.isFinite(port) || port <= 0) {
      throw new ContainerRuntimeError('docker inspect mapped host port', 1, out, '');
    }
    return port;
  }
  isAlreadyRemovedError(message: string): boolean {
...
class PodmanRuntime implements ContainerRuntime {
  ...
  async inspectMappedHostPort(handle: ContainerHandle, containerPort: number): Promise<number> {
    ...
    const port = Number(out);
    if (!Number.isFinite(port) || port <= 0) {
      throw new ContainerRuntimeError('podman inspect mapped host port', 1, out, '');
    }
    return port;
  }
  isAlreadyRemovedError(message: string): boolean {
...
async function tryRemove(bin: string, handle: ContainerHandle): Promise<ContainerRuntimeError | null> {
  try {
...
async function inspectDocker(id: string): Promise<ContainerInspect> {
  try {
    const json = await runShell('docker', ['inspect', '--format', '{{json .State}}', id]);
    const parsed = JSON.parse(json) as { Running?: boolean; Health?: { Status?: string }; Status?: string };
    return {
...
async function inspectPodman(id: string): Promise<ContainerInspect> {
  try {
    ...
    const arr = JSON.parse(json) as Array<{ State?: { Running?: boolean; HealthCheckStatus?: string }; Status?: string }>;
    const first = arr[0];
    return {
