...
// state-registry autotest suite. The fixture is owned by the
...
//            * wrongRoleCert      (CN: listener-admin, role=listener, team=team-a;
//                                   used to assert that a listener cert cannot
//                                   reach /admin/* even though it is trusted by
//                                   the CA)
...
const ServerExt = 'subjectAltName = @alt_names\n[alt_names]\nDNS.1 = localhost\nIP.1 = 127.0.0.1';

export interface ClientCertMaterial {
  /** Common Name on the cert. */
  cn: string;
  /** Service role the cert is supposed to authorize. */
  role: 'listener' | 'team-executor' | 'gateway' | 'system-executor' | 'wrong-role' | 'wrong-team';
...
async function runOpenssl(args: string[]): Promise<string> {
  return await new Promise<string>((resolve, reject) => {
    const child: ChildProcess = spawn(OpenSslBinary, args, { stdio: ['ignore', 'pipe', 'pipe'] });
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`openssl ${args.join(' ')} timed out after ${OpenSslWallClockMs}ms`));
    }, OpenSslWallClockMs);
    ...
    child.once('error', (err) => {
      clearTimeout(timer);
      if (isENOENT(err)) {
        reject(new Error(`openssl binary not found on PATH; required for v0002.20 ephemeral TLS`));
        return;
      }
      reject(err);
    });
    child.once('close', (code, signal) => {
      ...
      reject(new Error(`openssl ${args.join(' ')} exited code=${String(code)} signal=${String(signal)} stderr=${err.trim()}`));
    });
  });
}
...
async function writeExtFile(dir: string, name: string, ext: string): Promise<string> {
...
async function writeCaConfig(dir: string, certPath: string, keyPath: string): Promise<string> {
...
async function generateCa(dir: string): Promise<{ certPath: string; keyPath: string }> {
...
export async function createEphemeralTlsMaterial(
  ...
  const trustedListener: ClientCertMaterial = {
    ...(await signClientCert(tempDir, ca.certPath, ca.keyPath, 'listener-team-a', 'listener-team-a', lifetimeDays)),
    role: 'listener',
    teamId: 'team-a',
  };
  const trustedTeamExecutor: ClientCertMaterial = {
    ...(await signClientCert(tempDir, ca.certPath, ca.keyPath, 'executor-team-a', 'executor-team-a', lifetimeDays)),
    role: 'team-executor',
    teamId: 'team-a',
  };
  const trustedGateway: ClientCertMaterial = {
    ...(await signClientCert(tempDir, ca.certPath, ca.keyPath, 'gateway-team-a', 'gateway-team-a', lifetimeDays)),
    role: 'gateway',
    teamId: 'team-a',
  };
  const wrongRoleCert: ClientCertMaterial = {
    ...(await signClientCert(tempDir, ca.certPath, ca.keyPath, 'listener-impersonator', 'listener-impersonator', lifetimeDays)),
    role: 'wrong-role',
    teamId: 'team-a',
  };
  const wrongTeamCert: ClientCertMaterial = {
    ...(await signClientCert(tempDir, ca.certPath, ca.keyPath, 'listener-team-b', 'listener-team-b', lifetimeDays)),
    role: 'wrong-team',
    teamId: 'team-b',
  };
  const expiredCert: ClientCertMaterial = {
    ...(await signExpiredClientCert(tempDir, ca.certPath, ca.keyPath, 'expired-client', 'expired-client')),
    role: 'listener',
    teamId: 'team-a',
  };
  ...
  const untrustedCaCert: ClientCertMaterial = {
    ...(await signClientCert(untrustedCaDir, untrustedCa.certPath, untrustedCa.keyPath, 'untrusted-client', 'untrusted-client', lifetimeDays)),
    role: 'listener',
    teamId: 'team-a',
  };
  ...
  // half-baked cert.
  const required = [
    ca.certPath, ca.keyPath,
    server.certPath, server.keyPath,
    trustedListener.certPath, trustedListener.keyPath,
    trustedTeamExecutor.certPath, trustedTeamExecutor.keyPath,
    trustedGateway.certPath, trustedGateway.keyPath,
    wrongRoleCert.certPath, wrongRoleCert.keyPath,
    wrongTeamCert.certPath, wrongTeamCert.keyPath,
    untrustedCaCert.certPath, untrustedCaCert.keyPath,
    expiredCert.certPath, expiredCert.keyPath,
  ];
  ...
  return {
    ...
    async cleanup(): Promise<void> {
      try {
        ...
        // best-effort; the temp dir lives under OS tmpdir and will
...
export async function httpsRequest(
  ...
  if (parsed.protocol !== 'https:') {
    throw new Error(`httpsRequest requires an https:// URL; received ${parsed.protocol}`);
  }
  ...
  return await new Promise<HttpsRequestResult>((resolve, reject) => {
    const req = nodeHttpsRequest(url, reqOptions, (res) => {
      ...
      res.on('end', () => {
        try {
          const peerCert = res.socket instanceof Object && 'getPeerCertificate' in res.socket
            ? (res.socket as { getPeerCertificate: () => { subject?: { CN?: string } } }).getPeerCertificate()
            : null;
    ...
    req.on('timeout', () => {
      req.destroy(new Error(`httpsRequest timed out after ${String(reqOptions.timeout)}ms url=${url}`));
    });
    if (options.body) {
...
// resolves with { ok: true } on a successful handshake, with
// { ok: false, ... } on a TLS-level error, and rejects only on
// non-TLS failure (e.g. ECONNREFUSED). The contract test uses this
// to prove that the server rejected the handshake before any HTTP
// body was exchanged.
export async function tlsHandshakeProbe(options: TlsHandshakeOptions): Promise<TlsHandshake> {
  ...
  return await new Promise<TlsHandshake>((resolve) => {
    ...
    sock.once('secureConnect', () => {
      ...
      finish({ ok: true, tlsErrorCode: null, clientCertRequired: false, message: 'secureConnect' });
    });
    sock.once('error', (err) => {
      const code = (err as { code?: string }).code ?? 'UNKNOWN';
      const message = (err as Error).message ?? '';
      const clientCertRequired = /certificate required/i.test(message)
