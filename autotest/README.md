...
Cross-service Playwright tests live under `autotest/state-registry/`.
...
```bash
cd autotest/state-registry
npm test
```

Generated HTML, JSON, API, video, and trace artifacts are written below
`autotest/reports/` and are ignored by Git. Open the HTML report with:
...
Implementation-active E2E case definitions live in `autotest/test-cases/`.
