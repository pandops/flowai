...
// types defined in executor_docker_openhands/internal/platform and the
...
import (
	...
	"github.com/flowai/platform/executor_docker_openhands/internal/platform"
)
...
type RegisterExecutorRequest struct {
	...
	Identity        string          `json:"-"` // deprecated fixture field; never serialized
...
// CreateExecutor POSTs the first-start registration and pins the generated
// executor_id only after the Registry returns a successful response.
func (c *Client) CreateExecutor(ctx context.Context, body RegisterExecutorRequest) (*ExecutorRecord, error) {
	if c.Identity().ExecutorID != "" {
		return nil, errors.New("stateregistryclient: executor_id already exists")
	}
	out := &ExecutorRecord{}
	if err := c.doJSON(ctx, http.MethodPost, c.baseURL+"/executors", body, out, ""); err != nil {
		return nil, err
	}
	if out.ExecutorID == "" {
		return nil, errors.New("stateregistryclient: registration response omitted executor_id")
	}
	c.mu.Lock()
	c.identity.ExecutorID = out.ExecutorID
	c.mu.Unlock()
	return out, nil
}
...
func (c *Client) RegisterExecutor(ctx context.Context, body RegisterExecutorRequest) (*ExecutorRecord, error) {
	...
	if identity.ExecutorID == "" {
		return nil, errors.New("stateregistryclient: executor_id is required for refresh")
	}
	u := c.baseURL + "/executors/" + url.PathEscape(identity.ExecutorID)
	out := &ExecutorRecord{}
...
func (c *Client) applyIdentityHeaders(req *http.Request) {
	identity := c.Identity()
	req.Header.Set(platform.V0002HeaderExecutorID, identity.ExecutorID)
	req.Header.Set(platform.V0002HeaderRole, roleForScope(identity.Scope))
	if identity.TeamID != "" {
		req.Header.Set(platform.V0002HeaderTeamID, identity.TeamID)
	}
