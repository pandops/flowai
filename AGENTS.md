...
│   ├── Containerfile              # production and test runtime image
...
│   ├── migrations/
│   └── test/
└── web-ui/                        # standalone operator Web UI service
    ├── Containerfile
    ├── cmd/web-ui/
    ├── configs/
    ├── internal/
    ├── web/                       # embedded browser artifact
    └── test/
...
9. **Every runtime service and concrete Executor is container-native.** Each
   directory under `svc/` and each concrete Executor directory under
   `executor/` MUST own a production `Containerfile` that builds a runnable,
   self-contained image for that exact service. Production, integration, and
   cross-service E2E environments MUST start State Registry, Web UI, and every
   concrete Executor from those built images; test harnesses MUST NOT replace
   a service container with a host-compiled binary or an in-process service.
   The Docker OpenHands Executor receives the explicitly scoped container
   runtime socket it needs to supervise sibling task containers; the K8s
   OpenHands Executor runs as its own Pod image. Unit tests MAY execute Go
   packages on the host when they do not start a runtime service. In-process
   fakes and the mocked proxy remain test fixtures, not production services,
   and do not weaken this image-execution rule for real services.
...
- [**API Gateway**](openspec/specs/api-gateway/spec.md) — Web UI's sole backend; its accepted provider-neutral authentication contract is defined by [**Auth**](openspec/specs/auth/spec.md) and was delivered by archived change `v0007-auth`.
