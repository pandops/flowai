...
function expectTeamBound(
  ...
  expect(identity.teamId, `${identity.role} must carry teamId`).toBe(
    expectedTeamId,
  );
  const headers = identity.attach();
  expect(
    headers["X-FlowAI-Team-Id"],
    `${identity.role} must emit X-FlowAI-Team-Id`,
  ).toBe(expectedTeamId);
}
...
function expectTeamless(identity: IdentityContext): void {
  expect(
    identity.teamId,
    `${identity.role} must NOT carry teamId`,
  ).toBeUndefined();
  const headers = identity.attach();
  expect(
    headers["X-FlowAI-Team-Id"],
...
test.describe("state-registry identity fixtures", () => {
  ...
  test("system administrator carries no team binding", () => {
    ...
    expect(
      systemAdministrator().attach()["X-FlowAI-Admin-Subject"],
    ).toBeTruthy();
  });
  ...
  test("attach() returns an immutable header snapshot", () => {
    ...
    expect(() => {
      (headers as Record<string, string>)["X-FlowAI-Team-Id"] =
        "team-impersonator";
    }).toThrow();
