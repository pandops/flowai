...
test("v0002.1 listener persists durable pending task before acknowledgement; REQUIRED non-null task_type_id; required_tag is server-derived", async () => {
  ...
  const {
    ...
    taskType,
    listenerIdentity,
  } = await bootstrapTeam(admin, worker.baseUrl, {
  ...
  try {
    await test.step("POST /v1/tasks commits a pending task with REQUIRED non-null task_type_id", async () => {
      ...
      expect(
        resp.status(),
        "listener ingestion MUST return 201 before any acknowledgement",
      ).toBe(201);
      const body = await resp.json();
      createdTaskId = body.task_id;
      expect(
        typeof createdTaskId,
        "ingestion response carries the canonical task_id",
      ).toBe("string");
      expect(
        body.team_id,
        "immutable team_id comes from the registered team",
      ).toBe(team.team_id);
      expect(
        body.task_type_id,
        "REQUIRED non-null same-team task_type_id must be persisted",
      ).toBe(taskType.task_type_id);
      expect(
        body.current_state,
        "ingestion commits current_state = pending without lifecycle events",
      ).toBe("pending");
      expect(
        body.required_tag,
        "required_tag is server-derived from task_type.execution_tag, NOT listener-supplied",
      ).toBe("openhands");
      expect(
        body.source_system_id,
        "immutable source_system_id comes from the registered source system",
      ).toBe(sourceSystem.source_system_id);
      expect(body.source_id, "source_id is persisted as supplied").toBe(
        "JIRA-142",
      );
      expect(
        body.owner_command_id,
        "owner_command_id is NULL before claim",
      ).toBeNull();
      ...
      expect(
        body.resolved_image,
        "resolved_image is NULL before claim",
      ).toBeNull();
      expect(body.image_source, "image_source is NULL before claim").toBeNull();
      expect(
        body.ingested_at,
        "ingested_at is set once at ingestion",
      ).toBeTruthy();
    });
    ...
    await test.step("trusted Gateway re-read confirms the durable pending task with no event history", async () => {
      expect(
        createdTaskId,
        "gateway re-read requires the real task_id",
      ).not.toBeNull();
      ...
      try {
        ...
        expect(
          view.status(),
          "trusted Gateway reads the ingested task with 200",
        ).toBe(200);
        const viewBody = (await view.json()) as Record<string, unknown>;
        expect(
          viewBody["task_id"],
          "Gateway read returns the same task_id",
        ).toBe(taskId);
        expect(
          viewBody["current_state"],
          "Gateway read shows current_state = pending",
        ).toBe("pending");
        expect(
          viewBody["required_tag"],
          "Gateway read shows server-derived required_tag",
        ).toBe("openhands");
        expect(
          viewBody["owner_command_id"],
          "Gateway read shows owner_command_id is NULL",
        ).toBeNull();
        expect(
          viewBody["executor_id"],
          "Gateway read shows executor_id is NULL",
        ).toBeNull();
        expect(
          viewBody["claimed_at"],
          "Gateway read shows claimed_at is NULL",
        ).toBeNull();
        expect(
          viewBody["resolved_image"],
          "Gateway read shows resolved_image is NULL",
        ).toBeNull();
        expect(
          viewBody["image_source"],
          "Gateway read shows image_source is NULL",
        ).toBeNull();
        ...
        expect(
          events.status(),
          "trusted Gateway reads the ordered event history with 200",
        ).toBe(200);
        const eventsBody = (await events.json()) as { items: unknown[] };
        expect(
          Array.isArray(eventsBody.items),
          "event history returns an items array",
        ).toBe(true);
        expect(
          eventsBody.items.length,
          "no lifecycle event is appended at ingestion",
        ).toBe(0);
      } finally {
        await gwApi.dispose();
...
test("v0002.2 listener dedup returns the existing canonical pending task for same (team_id, source_system_id, source_id)", async () => {
  ...
  try {
    await test.step("first POST /v1/tasks creates the canonical pending task", async () => {
      ...
      expect(
        typeof firstTaskId,
        "first ingestion returns an immutable task_id",
      ).toBe("string");
      expect(
        first.current_state,
        "first ingestion commits current_state = pending",
      ).toBe("pending");
      expect(
        first.task_type_id,
        "first ingestion persists the same-team task_type_id",
      ).toBe(taskType.task_type_id);
    });
    ...
    await test.step("same-team retry returns the same canonical task_id and appends no new event", async () => {
      ...
      expect(retry.task_id, "retry returns the SAME canonical task_id").toBe(
        firstTaskId,
      );
      expect(retry.current_state, "retry does not reset task state").toBe(
        "pending",
      );

      const events = await gwApi.get(
        `/v1/tasks/${encodeURIComponent(firstTaskId as string)}/events`,
      );
      expect(events.status(), "event history read returns 200").toBe(200);
      const eventsBody = (await events.json()) as { items: unknown[] };
      expect(eventsBody.items.length, "retry appends no lifecycle event").toBe(
        0,
      );
    });
  } finally {
...
test("v0002.32 team-owned durable state survives restart (ingested_at, claim, events, control)", async () => {
  ...
  await test.step("listener retry identifies the canonical task with no event append", async () => {
    ...
    expect(retry.task_id, "retry resolves to the SAME canonical task_id").toBe(
      initialTask.task_id,
    );
    expect(retry.ingested_at, "retry preserves the original ingested_at").toBe(
      initialIngestedAt,
    );
  });
  ...
  await test.step("post-restart: durable task identity is unchanged", async () => {
    ...
    const gwApi = await gw.api(worker.baseUrl);
    try {
      const view = await gwApi.get(
        `/v1/tasks/${encodeURIComponent(initialTask.task_id)}`,
      );
      expect(
        view.status(),
        "durable task identity survives restart and Gateway reads it with 200",
      ).toBe(200);
      ...
      expect(viewBody.task_id, "task_id is immutable across restart").toBe(
        initialTask.task_id,
      );
      expect(
        viewBody.ingested_at,
        "ingested_at is unchanged across restart",
      ).toBe(initialIngestedAt);
      expect(
        viewBody.current_state,
        "task remains pending across restart",
      ).toBe("pending");

      const events = await gwApi.get(
        `/v1/tasks/${encodeURIComponent(initialTask.task_id)}/events`,
      );
      expect(events.status(), "event history is reachable after restart").toBe(
        200,
      );
      const eventsBody = (await events.json()) as { items: unknown[] };
      expect(
        eventsBody.items.length,
        "no event was appended at ingestion and no new event was appended across restart",
      ).toBe(0);
    } finally {
      await gwApi.dispose();
    }
...
test("v0002.33 cross-team dedup is independent: identical source_id lives independently per team", async () => {
  ...
  expect(
    ingestB.team_id,
    "team-b owns its OWN canonical task with the same source_id",
  ).toBe(bsB.admin.team_id);
  expect(
    ingestB.task_type_id,
    "team-b task references team-b task_type_id",
  ).toBe(bsB.taskType.task_type_id);
  expect(
    ingestB.task_id,
    "team-a and team-b tasks are distinct canonical rows",
  ).not.toBe(ingestA.task_id);
  expect(
    ingestA.source_system_id,
    "team-a task references team-a source_system_id",
  ).toBe(bsA.sourceSystem.source_system_id);
  expect(
    ingestB.source_system_id,
    "team-b task references team-b source_system_id",
  ).toBe(bsB.sourceSystem.source_system_id);
});
...
test("v0002.65 ingestion commits pending with no event; missing/foreign task_type_id and listener-supplied required_tag are rejected", async () => {
  ...
  try {
    await test.step("happy path ingestion returns 201 pending with NO event", async () => {
      ...
      expect(
        body.current_state,
        "happy-path commits current_state = pending",
      ).toBe("pending");
      expect(
        body.required_tag,
        "required_tag is server-derived from task_type.execution_tag",
      ).toBe("openhands");
      expect(
        body.ingested_at,
        "ingested_at is set once and is immutable",
      ).toBeTruthy();
      expect(
        body.owner_command_id,
        "owner_command_id is NULL on commit",
      ).toBeNull();
      ...
      expect(
        body.resolved_image,
        "resolved_image is NULL on commit",
      ).toBeNull();
    ...
    await test.step("Gateway read confirms zero events in the ordered history", async () => {
      expect(
        happyTaskId,
        "event verification requires the committed task_id",
      ).not.toBeNull();
    ...
    await test.step("negative control: missing task_type_id is rejected with 400 and no persistence", async () => {
      ...
      expect(
        resp.status(),
        "missing REQUIRED task_type_id is rejected without persistence",
      ).toBe(400);
    });
    ...
    await test.step("negative control: foreign-team task_type_id is rejected with 403 without persistence", async () => {
      ...
      expect(
        resp.status(),
        "foreign-team task_type_id is rejected with 403 team_mismatch without persistence",
      ).toBe(403);
    });
    ...
    await test.step("negative control: listener-supplied required_tag is rejected with 400", async () => {
      ...
      expect(
        resp.status(),
        "listener-supplied required_tag is rejected with 400 listener_supplied_required_tag_forbidden",
      ).toBe(400);
    });
  } finally {
    await listenerApi.dispose();
...
test("v0002.66 source_id dedupe uses (team_id, source_system_id, source_id) and rejects cross-team source-system reuse", async () => {
  ...
  await test.step("team-b listener submitting under team-a source system is rejected with 403", async () => {
    ...
    try {
      ...
      expect(
        resp.status(),
        "cross-team source-system identity is rejected with 403 without persistence",
      ).toBe(403);
    } finally {
      await api.dispose();
    }
