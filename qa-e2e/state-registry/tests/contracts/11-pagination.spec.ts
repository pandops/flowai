...
test("section 3b opaque cursors advance and reject tampering, changed scope, endpoint, and filters", async () => {
  ...
  for (const executionTag of ["alpha", "beta"]) {
    ...
    taskTypeIDs.push(
      ((await response.json()) as { task_type_id: string }).task_type_id,
    );
  }

  const firstTagPage = await adminAPI.get(
    `/admin/tags?team_id=${teamID}&limit=1`,
  );
  ...
  expect(firstTagBody.items.map((item) => item.execution_tag)).toEqual([
    "alpha",
  ]);
  ...
  const secondTagBody = (await secondTagPage.json()) as {
    items: Array<{ execution_tag: string }>;
  };
  expect(secondTagBody.items.map((item) => item.execution_tag)).toEqual([
    "beta",
  ]);
  ...
  const taskRows = [
    {
      id: `task-${suffix}-a`,
      source: `source-${suffix}-a`,
      at: "2026-01-01T00:00:00Z",
    },
    {
      id: `task-${suffix}-b`,
      source: `source-${suffix}-b`,
  ...
  expect(firstTaskBody.items.map((item) => item.task_id)).toEqual([
    taskRows[1]!.id,
  ]);
  expect(firstTaskBody.page.next_cursor).toEqual(expect.any(String));

  const nextCursor = firstTaskBody.page.next_cursor!;
  const secondTaskPage = await gatewayAPI.get(
    `/v1/tasks?limit=1&cursor=${encodeURIComponent(nextCursor)}`,
  );
  expect(secondTaskPage.status()).toBe(200);
  const secondTaskBody = (await secondTaskPage.json()) as {
    items: Array<{ task_id: string }>;
  };
  expect(secondTaskBody.items.map((item) => item.task_id)).toEqual([
    taskRows[0]!.id,
  ]);

  const tampered = await gatewayAPI.get(
    `/v1/tasks?limit=1&cursor=${encodeURIComponent(tamper(nextCursor))}`,
  );
  expect(tampered.status()).toBe(400);
  expect(((await tampered.json()) as { code: string }).code).toBe(
    "invalid_pagination",
  );
  ...
  const otherGatewayAPI = await gatewayFor({
    teamId: `other-${suffix}`,
    operatorId: `operator-${suffix}`,
  }).api(worker.baseUrl);
  const changedScope = await otherGatewayAPI.get(
    `/v1/tasks?limit=1&cursor=${encodeURIComponent(nextCursor)}`,
  );
  expect(changedScope.status()).toBe(400);

  const crossEndpoint = await adminAPI.get(
    `/admin/tasks?limit=1&cursor=${encodeURIComponent(nextCursor)}`,
  );
