// Infrastructure smoke for the state-registry qa-e2e harness. The test
// verifies in order:
...
test.describe("state-registry harness smoke", () => {
  test("infrastructure healthy and protected POST /admin/teams creates a team", async () => {
    ...
    expect(
      livez.status,
      "livez must report 200 when the process is alive",
    ).toBe(200);
    ...
    expect(
      readyz.status,
      "readyz must report 200 once postgres + AES key are present",
    ).toBe(200);
    ...
    expect(
      readyzBody.dependencies?.postgres,
      "postgres dependency must be reachable",
    ).toBe(true);
    expect(
      readyzBody.dependencies?.aes_key,
      "AES key dependency must be present",
    ).toBe(true);
  ...
  test("restart preserves PostgreSQL container handle while replacing the Go process", async () => {
    ...
    expect(
      afterDecrypt.raw,
      "decrypt-operation counter must remain zero after restart",
    ).toBe(0);
    const readyz = await fetch(`${worker.baseUrl}/v1/readyz`);
    expect(readyz.status, "readiness must return to 200 after restart").toBe(
      200,
    );
    const staleLivez = await fetch(`${beforeBaseUrl}/v1/livez`).catch(
      () => null,
    );
    if (staleLivez !== null) {
      expect(
        staleLivez.status,
        "pre-restart baseUrl must not serve the new process",
      ).not.toBe(200);
    }
    expect(
      worker.baseUrl,
      "baseUrl must reflect the current process after restart",
    ).not.toBe(beforeBaseUrl);
