...
function listStateRegistryContainers(): string[] {
  try {
    const out = execSync(
      'podman ps -a --format "{{.Names}}" --filter "name=state-registry-pg-"',
      {
...
function listStateRegistryGoProcesses(): string[] {
  try {
    const out = execSync("ps -eo pid,command", {
      stdio: ["ignore", "pipe", "pipe"],
    });
    return out
      .toString("utf-8")
      ...
      .filter((line) => {
        ...
        if (line.includes("qa-e2e/state-registry/")) {
          return false;
        }
        return (
    ...
    for (const line of out.toString("utf-8").split("\n")) {
      if (
        ...
        (line.includes("go run") ||
          line.includes("cmd/state-registry") ||
          line.includes("state-registry-bin-")) &&
        !line.includes("qa-e2e/state-registry/")
      ) {
...
test.describe("startRegistryWorker transactional behavior", () => {
  ...
  test("postgres-startup failure cleans up container (no leak)", async () => {
    ...
    try {
      await startRegistryWorker({
        postgresImage: "docker.io/library/this-image-does-not-exist:0",
      });
    } catch (err) {
      captured = err;
    }
    expect(
      captured,
      "startRegistryWorker should reject when postgres image is missing",
    ).toBeDefined();
    expect(String(captured)).toMatch(
      /state-registry worker failed during postgres-startup/,
    );
  ...
  test("bind-discovery failure cleans up process and container", async () => {
    ...
    expect(
      captured,
      "startRegistryWorker should reject when binary is missing",
    ).toBeDefined();
    expect(String(captured)).toMatch(
      /state-registry worker failed during|ENOENT/,
    );
    await new Promise((r) => setTimeout(r, 500));
    expect(listStateRegistryContainers()).toEqual(baseline);
    expect(listStateRegistryGoProcesses()).toEqual(baselineProcs);
  ...
  test("concurrent restart and teardown never leak a replacement process or container", async () => {
    ...
    try {
      ...
      expect(/lifecycle|torn down|worker failed during/i.test(message)).toBe(
        true,
      );
    }
    if (restartSucceeded) {
  ...
  test("teardown reports AggregateError(primary + kill failure + remove failure) deterministically", async () => {
    ...
    expect(
      captured,
      "teardown must reject when both kill and remove fail",
    ).toBeDefined();
    const err = captured as Error;
    expect(err.name).toMatch(/AggregateError/);
    const agg = err as AggregateError;
    expect(
      Array.isArray(agg.errors),
      "AggregateError.errors must be an array",
    ).toBe(true);
  ...
  test("startup failure with injected remove failure preserves primary+cleanup", async () => {
    ...
    expect(
      messages.some((m) =>
        /process-spawn|ENOENT|worker failed during/i.test(m),
      ),
    ).toBe(true);
    expect(
      messages.some((m) => /injected postgres remove failure/.test(m)),
