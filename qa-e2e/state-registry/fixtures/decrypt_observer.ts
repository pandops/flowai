// Decrypt-operation observer for the state-registry qa-e2e suite.
...
export async function snapshotDecryptOps(
  baseUrl: string,
): Promise<DecryptObservation> {
  const resp = await fetch(`${baseUrl}/v1/_test/decrypt-ops`);
  if (!resp.ok) {
    throw new Error(
      `failed to fetch decrypt-ops: status=${String(resp.status)}`,
    );
  }
