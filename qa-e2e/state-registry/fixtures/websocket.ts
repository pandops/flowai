// Concurrent WebSocket helper for the state-registry qa-e2e suite.
...
async function waitForClosed(ws: WebSocket, timeoutMs: number): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const timer = setTimeout(
      () =>
        finish(
          new Error(`WebSocket did not reach CLOSED within ${timeoutMs}ms`),
        ),
      timeoutMs,
    );
    const onClose = () => {
...
export async function openApiOnlySocket(
  url: string,
  timeoutMs = 5_000,
): Promise<OpenSocket> {
...
export async function openApiOnlySocketWithHeaders(
  ...
  try {
    ...
    throw new Error(
      `WebSocket construction failed for ${url}: ${(err as Error).message}`,
    );
  }
  ...
  openTimer = setTimeout(() => {
    settleOpen(() =>
      rejectOpen?.(new Error(`WebSocket open timeout url=${url}`)),
    );
  }, timeoutMs);
