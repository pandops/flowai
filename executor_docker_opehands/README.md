...
`executor_docker_opehands` is the concrete FlowAI worker for Docker and
...
```bash
env -u GOROOT go run ./executor_docker_opehands/cmd/executor_docker_opehands \
  -config executor_docker_opehands/configs/executor_docker_opehands.yaml
env -u GOROOT go test ./executor_docker_opehands/...
```
