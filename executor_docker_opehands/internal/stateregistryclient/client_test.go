...
// TestClientRejectsEmptyExecutorID asserts the v0009 contract:
// identity validation still rejects a missing Executor ID so the
// URL-path lookup cannot route to "unknown".
func TestClientRejectsEmptyExecutorID(t *testing.T) {
	_, err := stateregistryclient.New("http://example.com", stateregistryclient.Identity{
		Scope: "system",
	}, nil)
	if err == nil {
		t.Fatal("expected ExecutorID required")
	}
}
