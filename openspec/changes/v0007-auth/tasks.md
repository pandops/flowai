...
- [ ] Extend the repository E2E harness only under `qa-e2e/` with canonical auth-state fixtures, token issuance helpers, State Registry header/request spies, audit fixtures, and WebSocket event publishers; verify a harness smoke test passes before the first v0007 RED run.
