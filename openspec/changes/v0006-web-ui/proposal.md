...
- Run API Gateway in configured single-team no-auth bootstrap mode using exactly one deployment-configured non-empty `team_id`, canonical `team_name`, and exactly one deployment-configured non-empty bootstrap `operator_id`.
...
- Reject operator proxy traffic before creating any child request when the configured `operator_id`, `team_id`, or `team_name` is missing, empty, ambiguous, or otherwise invalid.
...
- Proxy state reads, live-event streams, control requests, and environment/secret writes only to State Registry; State Registry remains authoritative for resource ownership.
