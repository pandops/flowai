...
- [x] Keep all mocked Router, State Registry, and Env Registry data in memory only; durable PostgreSQL persistence is deferred to `v0002-state-registry`.
...
- [x] Implement the stateless Go service baseline: golang-standards layout (`cmd/`, `internal/`, `configs/`), `net/http` + `chi`, `slog` JSON logs, and YAML config.
...
- [x] Tests for mocked task server (all three in-memory surfaces): unit tests for `/v1/tasks` with and without `filter`, `/v1/executors/{executor_id}`, `/v1/executors/{executor_id}/events`, `/v1/tasks/{task_id}/events`, and `/v1/env` returning the shapes in the corresponding service OpenAPI files under `specs/openapi/`.
