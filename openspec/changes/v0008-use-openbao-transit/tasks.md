...
team_id, secret_id, version))` table do not exist; failure
      ...
      unavailable database fixture. Prove separately that v0002's
      `secret_versions_append_only` trigger rejects the envelope-only
      UPDATE the migration worker requires.
...
'pending'`. The migration SHALL be idempotent so re-running
      it is a no-op. In the same forward-only migration, replace
      `secret_versions_append_only` with an envelope-CAS UPDATE guard
      that permits only the documented envelope columns under the
      matching locked `in_progress` checkpoint and transaction-local
...
TestSecretVersionsDeleteStillRejected|
TestSecretVersionsIdentityUpdateRejected'`; require the
      ...
      baseline on a clean database; require envelope CAS to pass only
      under the matching active checkpoint claim and transaction-local
      marker, while unclaimed updates, identity updates, and every
      delete remain rejected.
...
version)`; the per-row checkpoint transactional advance
      ...
      as a source of truth; the byte-exact AAD handoff where local
      `Decrypt` receives v0002 legacy bytes
      `{team_id}\x00{secret_id}\x00{decimal_version}` and active
      `Encrypt` receives v0008 bytes
      `team_id=<team_id>\nsecret_id=<secret_id>\nversion=<uint64>\n`;
      the two byte strings SHALL differ for the same canonical triple;
...
TestMigrationCrashBetweenTransitAndCommit|
TestMigrationLocalDecryptUsesV0002AAD|
TestMigrationActiveEncryptUsesV0008AAD'`; require the
