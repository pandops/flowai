...
The mocked task server from `v0001-executor-docker` is removed from the Docker Executor path. Executor scheduling calls are redirected to Router surfaces with equivalent registration, polling, and status semantics plus durable Router persistence.
