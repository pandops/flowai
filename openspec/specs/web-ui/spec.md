...
- **THEN** the Web UI opens its WebSocket connection to the API Gateway instead of any backend service directly (maybe need direct websocket connection to container with agent)
...
- **THEN** the Web UI sends the write through the API Gateway to Secret Service
